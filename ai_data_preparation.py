...
def _format_ttest_results() -> Optional[str]:
    ...
    try:
        # Check multiple possible locations for t-test results
        ttest_data = None
        
        if 'ttest_results' in st.session_state:
            ttest_data = st.session_state.ttest_results
        elif 'analysis_results' in st.session_state and st.session_state.analysis_results.get('type') == 'ttest':
            ttest_data = st.session_state.analysis_results.get('result')
        
        if ttest_data:
            text = "\nt-TEST ANALYSIS:\n"
            text += "=" * 50 + "\n"
            
            # Format based on the structure of ttest results
            for key, value in ttest_data.items():
                if key == 'test_type':
                    text += f"Test Type: {value}\n"
                elif key == 't_stat':
                    text += f"t-statistic: {value:.4f}\n"
                elif key == 'p_value':
                    text += f"p-value: {value:.6f}\n"
                elif key == 'significant':
                    text += f"Statistically Significant: {'Yes' if value else 'No'}\n"
                elif key == 'interpretation':
                    text += f"Interpretation: {value}\n"
                elif key == 'confidence_interval':
                    text += f"Confidence Interval: {value}\n"
            
            text += "\n"
            return text
    except Exception as e:
        st.error(f"Error formatting t-test results: {str(e)}")
...
def _format_anova_results() -> Optional[str]:
    ...
    try:
        ...
        if 'anova_results' in st.session_state:
            anova_data = st.session_state.anova_results
        elif 'analysis_results' in st.session_state and st.session_state.analysis_results.get('type') == 'anova':
            anova_data = st.session_state.analysis_results.get('result')
        ...
        if anova_data:
            ...
            for key, value in anova_data.items():
                if key == 'f_stat':
                    text += f"F-statistic: {value:.4f}\n"
                elif key == 'p_value':
                    text += f"p-value: {value:.6f}\n"
                elif key == 'significant':
                    text += f"Statistically Significant: {'Yes' if value else 'No'}\n"
                elif key == 'anova_table' and isinstance(value, pd.DataFrame):
                    text += "ANOVA Table:\n"
                    text += value.to_string() + "\n"
                elif key == 'group_stats' and isinstance(value, pd.DataFrame):
        ...
        st.error(f"Error formatting ANOVA results: {str(e)}")
...
def _format_correlation_results() -> Optional[str]:
    ...
    try:
        ...
        if 'correlation_results' in st.session_state:
            corr_data = st.session_state.correlation_results
        elif 'analysis_results' in st.session_state and st.session_state.analysis_results.get('type') == 'correlation':
            corr_data = st.session_state.analysis_results.get('result')
        
        if corr_data:
            text = "\nCORRELATION ANALYSIS:\n"
            text += "=" * 50 + "\n"
            
            if 'corr_matrix' in corr_data and isinstance(corr_data['corr_matrix'], pd.DataFrame):
                text += "Correlation Matrix:\n"
                text += corr_data['corr_matrix'].to_string() + "\n\n"
            
            if 'significant_corrs' in corr_data and isinstance(corr_data['significant_corrs'], pd.DataFrame):
                text += "Significant Correlations:\n"
                text += corr_data['significant_corrs'].to_string() + "\n\n"
            
            if 'method' in corr_data:
                text += f"Correlation Method: {corr_data['method']}\n"
            
            text += "\n"
            return text
    except Exception as e:
        st.error(f"Error formatting correlation results: {str(e)}")
...
def _format_ml_results() -> Optional[str]:
    ...
    return None
        ...
        if chi_data:
            ...
            for key, value in chi_data.items():
                if key == 'chi2_stat':
                    text += f"Chi-square statistic: {value:.4f}\n"
                elif key == 'p_value':
                    text += f"p-value: {value:.6f}\n"
                elif key == 'significant':
                    text += f"Statistically Significant: {'Yes' if value else 'No'}\n"
                elif key == 'contingency_table' and isinstance(value, pd.DataFrame):
        ...
        st.error(f"Error formatting chi-square results: {str(e)}")
...
def _format_multi_sheet_analysis() -> Optional[str]:
    ...
    try:
        if 'multi_sheet_analysis' in st.session_state:
            multi_data = st.session_state.multi_sheet_analysis
            
            text = "\nMULTI-SHEET ANALYSIS:\n"
            text += "=" * 50 + "\n"
            
            if 'master_sheet' in multi_data:
                text += f"Master Sheet: {multi_data['master_sheet']}\n"
            
            if 'sheet_relationships' in multi_data:
                text += "Sheet Relationships:\n"
                for key, value in multi_data['sheet_relationships'].items():
                    text += f"- {key}: {value}\n"
            
            text += "\n"
            return text
    except Exception as e:
        st.error(f"Error formatting multi-sheet analysis: {str(e)}")
...
def _format_ml_results() -> Optional[str]:
    ...
    try:
        if 'ml_results' in st.session_state:
            ...
            if 'model_type' in ml_data:
                text += f"Model Type: {ml_data['model_type']}\n"
            
            if 'performance_metrics' in ml_data:
                text += "Performance Metrics:\n"
                for metric, value in ml_data['performance_metrics'].items():
                    text += f"- {metric}: {value}\n"
            
            if 'feature_importance' in ml_data:
                text += "Feature Importance:\n"
                text += str(ml_data['feature_importance']) + "\n"
            
            text += "\n"
            return text
    except Exception as e:
        st.error(f"Error formatting ML results: {str(e)}")
