...
including multi-sheet analysis and cross-sheet ML model training
"""
...
def analyze_data_structure(df):
    ...
    # For automated analysis, always detect all patterns
    # This is the ML-based automated analysis which should identify all potential patterns
...
def detect_calculation_patterns(df, skip_pre_post_detection=False):
    """
    ...
    df : pd.DataFrame
        The dataframe to analyze
    skip_pre_post_detection : bool
        If True, don't automatically detect pre/post pairs
        This is only used for the manual descriptive analysis to respect user configuration
    """
    patterns = []
    ...
    # Always detect pre/post pairs for automated analysis
    # The skip_pre_post_detection parameter is only used for manual descriptive analysis
...
def identify_master_sheet(sheets_data):
    """
    Identify which sheet is likely the master sheet in a multi-sheet workbook
    ...
    --------
    str, dict
        Name of the master sheet and information about why it was identified as master
    """
    if not sheets_data:
        return None, {"error": "No sheets provided"}
    ...
    for sheet_name, df in sheets_data.items():
        ...
        # 1. Check if "master" is in the sheet name (strong indicator)
        if "master" in sheet_name.lower():
            score += 10
            info["master_in_name"] = True
        
        # 2. Look for summary or metadata-type columns
        id_columns = [col for col in df.columns if (
            'id' in col.lower() or 
            'code' in col.lower() or
            'number' in col.lower() or
            'index' in col.lower()
        )]
        if id_columns:
            score += len(id_columns)
            info["id_columns"] = id_columns
        
        # 3. Check if this sheet has many categorical/system columns
        cat_cols = df.select_dtypes(include=['object', 'category']).columns.tolist()
        system_cols = [col for col in df.columns if (
            'system' in col.lower() or 
        ...
        if system_cols:
            score += len(system_cols) * 2
            info["system_columns"] = system_cols
        
        # 4. Check for sheets with fewer rows but more columns (often master/summary sheets)
        if len(df) < 100 and len(df.columns) > 10:
            score += 3
            info["compact_format"] = True
        
        # 5. First sheet bias (slightly favor the first sheet as it's often a summary or master)
        if list(sheets_data.keys()).index(sheet_name) == 0:
            score += 1
            info["first_sheet"] = True
        
        sheet_scores[sheet_name] = {"score": score, "info": info}
    
    # Get the sheet with the highest score
    if not sheet_scores:
        return None, {"error": "Could not score sheets"}
    
    best_sheet = max(sheet_scores.items(), key=lambda x: x[1]["score"])
    return best_sheet[0], best_sheet[1]

def analyze_sheet_relationships(sheets_data):
    """
    Analyze relationships between sheets in a multi-sheet workbook
    ...
    # Find common columns between pairs of sheets
    sheet_names = list(sheets_data.keys())
    for i, sheet1 in enumerate(sheet_names):
        relationships["sheet_similarities"][sheet1] = {}
        df1 = sheets_data[sheet1]
        cols1 = set(df1.columns)
        
        for j, sheet2 in enumerate(sheet_names[i+1:], i+1):
            df2 = sheets_data[sheet2]
            ...
            if common:
                relationships["common_columns"][f"{sheet1}__{sheet2}"] = list(common)
                
                # Calculate Jaccard similarity for sheet columns
                similarity = len(common) / len(cols1.union(cols2))
                relationships["sheet_similarities"][sheet1][sheet2] = similarity
                
                # Check for potential foreign key relationships
                for col in common:
                    # If values in sheet1[col] are subset of sheet2[col] or vice versa
                    vals1 = set(df1[col].dropna().unique())
                    vals2 = set(df2[col].dropna().unique())
                    
                    if vals1.issubset(vals2) and len(vals1) > 0:
                        relationships["potential_foreign_keys"].append({
                            "from_sheet": sheet1,
                            "to_sheet": sheet2,
                            "column": col,
                            "cardinality": f"{len(vals1)} -> {len(vals2)}"
                        })
                    elif vals2.issubset(vals1) and len(vals2) > 0:
                        relationships["potential_foreign_keys"].append({
                            "from_sheet": sheet2,
                            "to_sheet": sheet1,
                            "column": col,
                            "cardinality": f"{len(vals2)} -> {len(vals1)}"
                        })
    
    return relationships

def train_cross_sheet_ml_model(sheets_data, master_sheet_name=None, target_column=None):
    """
    Train a machine learning model using data from multiple sheets with a master sheet
    
    Parameters:
    -----------
    sheets_data : dict
        Dictionary of sheet names to dataframes
    master_sheet_name : str or None
        Name of the master sheet. If None, will attempt to identify automatically
    target_column : str or None
        Target column for supervised learning. If None, will attempt to identify automatically
        
    Returns:
    --------
    dict
        Dictionary with trained model and related information
    """
    if not sheets_data:
        return {"error": "No sheets provided"}
    ...
    # Step 1: Identify master sheet if not provided
    if master_sheet_name is None:
        master_sheet_name, master_info = identify_master_sheet(sheets_data)
        if master_sheet_name is None:
            return {"error": "Could not identify master sheet"}
        results["master_sheet"] = {
            "name": master_sheet_name,
            "selection_info": master_info
        }
    else:
        if master_sheet_name not in sheets_data:
            return {"error": f"Specified master sheet '{master_sheet_name}' not found in workbook"}
        results["master_sheet"] = {
            "name": master_sheet_name,
            "selection_info": {"user_specified": True}
        }
    
    master_df = sheets_data[master_sheet_name]
    
    # Step 2: Analyze relationships between sheets
    relationships = analyze_sheet_relationships(sheets_data)
    results["sheet_relationships"] = relationships
    
    # Step 3: Merge relevant data from output sheets into the master sheet
    # This is a complex process that depends on the specific structure of the data
    # For now, we'll create a simplified merged dataset for modeling
    merged_data = master_df.copy()
    
    for sheet_name, df in sheets_data.items():
        if sheet_name == master_sheet_name:
            continue
        
        # Check for common columns with master sheet
        common_key = f"{master_sheet_name}__{sheet_name}"
        reversed_key = f"{sheet_name}__{master_sheet_name}"
        
        if common_key in relationships["common_columns"]:
            common_cols = relationships["common_columns"][common_key]
        elif reversed_key in relationships["common_columns"]:
            common_cols = relationships["common_columns"][reversed_key]
        else:
            # No common columns, skip this sheet
            continue
        
        # Pick the first common column as a merge key (this is simplified)
        if common_cols:
            merge_col = common_cols[0]
            
            # Create a simplified suffix for the merged columns
            sheet_suffix = f"_{sheet_name}"
            
            # Merge data, keeping only the master sheet's version of common columns
            # and adding a suffix to other columns from the current sheet
            sheet_cols_to_merge = [col for col in df.columns if col not in common_cols or col == merge_col]
            if sheet_cols_to_merge:
                try:
                    temp_df = df[sheet_cols_to_merge].copy()
                    
                    # Add suffix to all columns except the merge column
                    rename_cols = {col: f"{col}{sheet_suffix}" for col in temp_df.columns if col != merge_col}
    ...
    results["merged_data_shape"] = merged_data.shape
    
    # Step 4: Identify target column if not provided
    if target_column is None:
        # Try to identify potential target columns from output sheets
        # Look for columns that might be outputs (often in the later sheets)
        potential_targets = []
        
        # Check for columns with "output", "result", "yield", etc. in name
        output_keywords = ["output", "result", "yield", "efficiency", "total", "final"]
        for col in merged_data.columns:
            col_lower = str(col).lower()
            if any(keyword in col_lower for keyword in output_keywords):
                if pd.api.types.is_numeric_dtype(merged_data[col]):
                    potential_targets.append(col)
        
        # If no clear targets found, use numeric columns from non-master sheets
        if not potential_targets:
            for col in merged_data.columns:
                if "_" in col and pd.api.types.is_numeric_dtype(merged_data[col]):
                    potential_targets.append(col)
        
        # Pick the first potential target
        if potential_targets:
            target_column = potential_targets[0]
            results["target_column"] = {
                "name": target_column,
                "auto_selected": True,
                "alternatives": potential_targets[1:5]  # List a few alternatives
            }
        else:
            return {"error": "Could not identify a suitable target column for ML model"}
    else:
        if target_column not in merged_data.columns:
            return {"error": f"Specified target column '{target_column}' not found in merged data"}
        results["target_column"] = {
            "name": target_column,
            "user_specified": True
        }
    ...
    feature_cols = [col for col in merged_data.columns 
                   if col != target_column and not merged_data[col].isnull().all()]
    
    # Handle different data types for ML
    ...
    # Step 6: Create a simple model based on numeric features only (for simplicity)
    if numeric_cols:
        try:
            # Prepare a simplified dataset with just numeric features
            X = merged_data[numeric_cols].fillna(0)  # Simple imputation
            y = merged_data[target_column]
            
            # Determine if regression or classification task
            target_unique_vals = y.nunique()
            if target_unique_vals <= 10:  # Arbitrary threshold
                task_type = 'classification'
            ...
            results["model_info"]["type"] = task_type
            results["model_info"]["algorithm"] = type(model).__name__
            
            # Train the model
            model.fit(X, y)
            
            # Determine model importance
            ...
            results["model_info"]["feature_importance"] = sorted_importance
            results["model_info"]["top_features"] = [f[0] for f in sorted_importance[:5]]
        ...
        return {
            "error": "No suitable numeric features found for modeling",
            "partial_results": results
        }

def analyze_multi_sheet_excel(sheets_data):
    """
    Analyze multiple sheets in an Excel file and identify relationships and patterns
    ...
    for sheet_name, df in sheets_data.items():
        ...
        sheet_analysis = analyze_data_structure(df)
        results["sheets"][sheet_name] = sheet_analysis
    
    # Identify master sheet
    master_sheet, master_info = identify_master_sheet(sheets_data)
    results["master_sheet"] = {
        "name": master_sheet,
        "info": master_info
    }
    ...
    relationships = analyze_sheet_relationships(sheets_data)
    results["cross_sheet_analysis"]["relationships"] = relationships
    
    # Calculate overall sheet similarity network
    sheet_similarity_network = {}
    for sheet1, similarities in relationships["sheet_similarities"].items():
        for sheet2, similarity in similarities.items():
            sheet_similarity_network[f"{sheet1}__{sheet2}"] = similarity
    
    results["cross_sheet_analysis"]["sheet_similarity_network"] = sheet_similarity_network
    
    # Try to identify sheet hierarchy/dependency
    if master_sheet:
        sheet_hierarchy = {"root": master_sheet, "branches": {}}
        master_df = sheets_data[master_sheet]
        
        # For each sheet, try to find its relationship to the master
        for sheet_name, df in sheets_data.items():
            if sheet_name == master_sheet:
                continue
            
            common_key = f"{master_sheet}__{sheet_name}"
            reversed_key = f"{sheet_name}__{master_sheet}"
            
            if common_key in relationships["common_columns"]:
                common_cols = relationships["common_columns"][common_key]
                sheet_hierarchy["branches"][sheet_name] = {
                ...
                # No direct relationship to master, look for indirect relationships
                sheet_hierarchy["branches"][sheet_name] = {
                    "relationship": "indirect"
                }
        
        results["cross_sheet_analysis"]["sheet_hierarchy"] = sheet_hierarchy
