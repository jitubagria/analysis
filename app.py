...
from visualization_utils import (create_histogram, create_boxplot, create_scatterplot,
                                 create_correlation_heatmap, create_distribution_plot, generate_dynamic_plotly_chart)
...
def export_charts_to_word():
    ...
    if 'chart_favorites' not in st.session_state or not st.session_state.chart_favorites:
        doc.add_paragraph(
            "No charts have been favorited. Please mark charts with the heart icon to include them in the export.")
    ...
    for chart_id, chart_item in st.session_state.chart_favorites.items():
        ...
        try:
            ...
            export_fig.write_image(img_bytes, format="png", scale=2)
            img_bytes.seek(0)
...
def emoji_header(title, emoji_type):
    emoji = random.choice(EMOJI_DICT.get(emoji_type, ["📊"])) if isinstance(
        EMOJI_DICT.get(emoji_type, ["📊"]), list) else EMOJI_DICT.get(emoji_type, "📊")
...
def display_mobile_friendly_dataframe(df, title="Dataset", use_container_width=True, height=None):
    ...
    if st.checkbox(f"Enable mobile-friendly view for {title}", value=False, key=f"mobile_view_{title}"):
        ...
        st.dataframe(
            display_df, use_container_width=use_container_width, height=height)
    else:
        # Show all columns (desktop view)
        st.dataframe(
            df, use_container_width=use_container_width, height=height)
...
def main():
    ...
    with st.sidebar:
        ...
        st.markdown(
            "Configure AI providers for comprehensive analysis summaries")

        # AI Provider Selection
        ai_provider = st.selectbox(
            "AI Provider",
            ["Google Gemini", "OpenAI (Coming Soon)",
        ...
        if ai_provider == "Google Gemini":
            ...
            if gemini_model == "gemini-1.5-flash":
                st.info(
                    "Flash model: Fast responses, optimized for speed and efficiency")
            ...
            st.markdown(
                "[Get API Key](https://aistudio.google.com/app/apikey) | [Documentation](https://ai.google.dev/docs)")
    ...
    steps = [
        ...
        {"title": "9. Drag Drop Visualization", "step": 9}  # New Step
    ]
    ...
    selected_step = st.selectbox(
        "Go to step:",
        options=[
            f"{step['step']}. {step['title'].split('. ')[1]}" for step in steps],
    ...
    for i, (col, step) in enumerate(zip(cols, steps)):
        ...
        with col:
            if step_num == st.session_state.current_step:
                ...
                st.markdown(
                    f'<div style="background-color: #e6f2ff; padding: 10px; border-radius: 5px; text-align: center; font-weight: bold;">{step["title"]}</div>', unsafe_allow_html=True)
            else:
    ...
    if st.session_state.current_step == 1:
        ...
        with upload_col:
            ...
            uploaded_file = st.file_uploader(
                "Upload your data file", type=['csv', 'xlsx', 'xls'])
            ...
            if uploaded_file is not None:
                try:
                    ...
                    with st.spinner(""):
                        ...
                        success_emoji = random.choice(EMOJI_DICT["Success"])
                        st.success(
                            f"{success_emoji} Data loaded successfully! Found {df.shape[0]} rows and {df.shape[1]} columns")

                    # DEBUG: Check if the file already has Age Group columns
                    age_group_cols = [
                        col for col in df.columns if 'age group' in col.lower() or 'Age group' in col]
                    if age_group_cols:
                        st.warning(
                            f"Found existing Age Group columns in the file: {age_group_cols}")
                    ...
                    with dt_col1:
                        st.markdown(
                            "**🔢 Numeric columns:** " + str(len(df.select_dtypes(include=['int64', 'float64']).columns)))
                    with dt_col2:
                        st.markdown(
                            "**🔤 Text columns:** " + str(len(df.select_dtypes(include=['object']).columns)))
                    with dt_col3:
                        st.markdown(
                            "**📅 Date columns:** " + str(len(df.select_dtypes(include=['datetime64']).columns)))
                    with dt_col4:
                        st.markdown("**✓✗ Boolean columns:** " +
                                    str(len(df.select_dtypes(include=['bool']).columns)))

                    # Display dataframe in a mobile-friendly way
                    display_mobile_friendly_dataframe(
                        df, title="Uploaded Data Preview", height=600)
        ...
        with sample_col:
            ...
            st.write(
                "Don't have a dataset? Use our sample data to explore the application.")
            ...
            if st.button("Use Sample Data (Iris Dataset)"):
                ...
                display_mobile_friendly_dataframe(
                    iris_df, title="Raw Data Table (Iris Sample)", height=600)
        ...
        if st.session_state.data is not None:
            ...
            tab1, tab2 = st.tabs(
                ["Excel Drag & Drop Config", "Basic Column Selection"])
                ...
                if 'configuration_complete' in st.session_state and st.session_state.configuration_complete:
                    ...
                    if 'gender_column' in st.session_state and st.session_state.gender_column:
                        if st.session_state.gender_column['name'] not in st.session_state.selected_columns:
                            st.session_state.selected_columns.append(
                                st.session_state.gender_column['name'])
            ...
            with tab2:
                ...
                st.session_state.selected_columns = st.multiselect(
                    ...
                    all_cols,
                    default=st.session_state.selected_columns if st.session_state.selected_columns else all_cols[
                        :5]
                )
                ...
                if st.session_state.selected_columns:
                    ...
                    with col1:
                        ...
                        numerical_cols = [
                            col for col in st.session_state.selected_columns if pd.api.types.is_numeric_dtype(df[col])]
                        if numerical_cols:
                    ...
                    with col2:
                        ...
                        categorical_cols = [
                            col for col in st.session_state.selected_columns if not pd.api.types.is_numeric_dtype(df[col])]
                        if categorical_cols:
                    ...
                    missing_values = df[st.session_state.selected_columns].isnull(
                    ).sum()
                    if missing_values.sum() > 0:
                        ...
                        missing_strategy = st.selectbox(
                            ...
                            ["None (Keep as is)", "Drop rows with missing values",
                             "Fill with mean/mode"]
                        )
                        ...
                        if missing_strategy != "None (Keep as is)" and st.button("Apply Missing Value Strategy"):
                            if missing_strategy == "Drop rows with missing values":
                                df_clean = df.dropna(
                                    subset=st.session_state.selected_columns)
                                st.session_state.data = df_clean
                                st.success(
                                    f"Dropped rows with missing values. Remaining rows: {len(df_clean)}")
                                st.dataframe(
                                    df_clean[st.session_state.selected_columns].head())
                                ...
                                for col in st.session_state.selected_columns:
                                    if pd.api.types.is_numeric_dtype(df[col]):
                                        ...
                                        df_filled[col] = df_filled[col].fillna(
                                            fill_value)
                                        st.info(
                                            f"Filled '{col}' with mean: {fill_value:.2f}")
                                    else:
                                ...
                                st.dataframe(
                                    df_filled[st.session_state.selected_columns].head())
                    else:
                        st.write(
                    ...
                    dtypes_df = pd.DataFrame({
                        ...
                        # Convert dtype to string to prevent Arrow conversion issues
                        'Data Type': [str(df[col].dtype) for col in st.session_state.selected_columns]
                    })
                    st.table(dtypes_df)
                else:
                    st.warning(
                        "Please select at least one column for analysis.")
            ...
            if 'age_column' in st.session_state and st.session_state.age_column:
                ...
                age_group_cols = [
                    col for col in df.columns if 'age group' in col.lower()]
                if age_group_cols:
            ...
            if all_display_columns:
                ...
                def highlight_columns(x):
                    ...
                    age_group_cols = [col for col in x.columns if 'age group' in col.lower(
                    ) or 'generated age group' in col.lower()]
                    ...
                    if 'pair_areas' in st.session_state and st.session_state.pair_areas:
                        for idx, pair in enumerate(st.session_state.pair_areas):
                            ...
                            # Vary the shade for different pairs
                            green_intensity = 200 - (idx * 10) % 50
                            ...
                            # Vary the shade for different pairs
                            purple_intensity = 200 - (idx * 10) % 50
                ...
                with col1:
                    st.markdown(
                        '<div style="background-color: #e6f2ff; padding: 5px;">Master Columns</div>', unsafe_allow_html=True)
                with col2:
                    st.markdown(
                        '<div style="background-color: #ffffcc; padding: 5px;">Age Column</div>', unsafe_allow_html=True)
                with col3:
                    st.markdown(
                        '<div style="background-color: #ffebcc; padding: 5px;">Age Group Column</div>', unsafe_allow_html=True)
                with col4:
                    st.markdown(
                        '<div style="background-color: #ffdddd; padding: 5px;">Gender Column</div>', unsafe_allow_html=True)
                with col5:
                    st.markdown(
                        '<div style="background-color: rgba(144, 200, 144, 0.3); padding: 5px;">Pre Pairs</div>', unsafe_allow_html=True)
                with col6:
                    st.markdown(
                        '<div style="background-color: rgba(200, 144, 240, 0.3); padding: 5px;">Post Pairs</div>', unsafe_allow_html=True)
            else:
            ...
            if st.button("Continue to Data Analysis"):
                if not st.session_state.selected_columns:
                    st.error(
                        "Please select at least one column for analysis before continuing.")
                else:
        ...
        if st.session_state.data is not None:
            ...
            st.session_state.analysis_type = st.radio(
                ...
                index=analysis_options.index(
                    st.session_state.analysis_type) if st.session_state.analysis_type in analysis_options else 0
            )

            if st.session_state.analysis_type == "Descriptive Statistics":
                st.write(
                    "You'll get summary statistics for your selected columns, master parameter analysis, and pre/post analysis.")

            elif st.session_state.analysis_type == "Statistical Tests":
                st.write(
                    "You'll be able to perform hypothesis tests (t-tests, ANOVA) on your data.")
                ...
                if test_type == "t-Test":
                    ...
                    if not num_cols:
                        st.error(
                            "No numeric columns available for t-test analysis. Please include numeric columns in your selection.")
                    elif all_cols:
                        ...
                        if ttest_settings['test_type'] == "One-sample t-test":
                            ...
                            ttest_settings['mu'] = st.number_input(
                                "Population mean (μ₀):", value=0.0)
                            ttest_settings['alpha'] = st.slider(
                                "Significance level (α):", 0.01, 0.10, 0.05)

                        else:  # Two-sample t-test
                            ttest_settings['method'] = st.radio(
                                "Select method:",
                                ["Compare two columns",
                                    "Compare groups within a column"]
                            )
                            ...
                            if ttest_settings['method'] == "Compare two columns":
                                ...
                                remaining_num_cols = [
                                    col for col in num_cols if col != ttest_settings['col1']]
                                if remaining_num_cols:
                                    ...
                                    ttest_settings['equal_var'] = st.checkbox(
                                        "Assume equal variances", value=False)
                                    ttest_settings['alpha'] = st.slider(
                                        "Significance level (α):", 0.01, 0.10, 0.05, key="ttest_alpha2")
                                else:
                                    st.warning(
                                        "Need at least two numeric columns for two-sample t-test.")
                                ...
                                ttest_settings['group_col'] = st.selectbox(
                                    "Select grouping column:", all_cols)
                                # Get unique values for the selected grouping column
                                unique_groups = df[ttest_settings['group_col']].dropna(
                                ).unique().tolist()

                                if len(unique_groups) >= 2:
                                    ttest_settings['group1'] = st.selectbox(
                                        "Select first group:", unique_groups)
                                    # Filter out the first group
                                    remaining_groups = [
                                        g for g in unique_groups if g != ttest_settings['group1']]
                                    ttest_settings['group2'] = st.selectbox(
                                        "Select second group:", remaining_groups)
                                    ttest_settings['equal_var'] = st.checkbox(
                                        "Assume equal variances", value=False, key="ttest_eq_var2")
                                    ttest_settings['alpha'] = st.slider(
                                        "Significance level (α):", 0.01, 0.10, 0.05, key="ttest_alpha3")
                                else:
                                    st.warning(
                                        f"Need at least two groups in {ttest_settings['group_col']} for two-sample t-test.")
                    else:
                    ...
                    if not num_cols:
                        st.error(
                            "No numeric columns available for ANOVA analysis. Please include numeric columns in your selection.")
                    elif all_cols:
                        # Use only numeric columns for the dependent variable
                        ...
                        unique_groups = df[anova_settings['cat_col']].dropna(
                        ).unique()
                        if len(unique_groups) < 3:
                            st.warning(
                                f"ANOVA typically requires at least 3 groups, but '{anova_settings['cat_col']}' only has {len(unique_groups)} unique values.")

                        # Display the unique groups found
                        with st.expander("View factor levels"):
                            st.write(
                                f"Factor '{anova_settings['cat_col']}' has these unique values:")
                            st.write(", ".join([str(g)
                                     for g in unique_groups]))

                        anova_settings['alpha'] = st.slider(
                            "Significance level (α):", 0.01, 0.10, 0.05, key="anova_alpha")
                    else:
                    ...
                    cat_cols = [col for col in all_cols if not pd.api.types.is_numeric_dtype(
                        df[col]) or df[col].nunique() < 10]
                    ...
                    if not cat_cols:
                        st.error(
                            "No categorical columns identified for Chi-Square Test. Please include categorical columns in your selection.")
                    elif len(cat_cols) < 2:
                        st.error(
                            "Chi-Square Test requires at least two categorical variables to test for association.")
                    else:
                        ...
                        with col2:
                            ...
                            # Show the unique values in this column
                            unique_vals2 = df[chi2_settings['col2']].dropna(
                            ).unique()
                        ...
                        if preview_table.shape[0] > 10 or preview_table.shape[1] > 10:
                            st.info(
                                f"Table is large ({preview_table.shape[0]}×{preview_table.shape[1]}). Showing partial view.")
                ...
                st.write(
                    "You'll analyze relationships between numerical variables.")
                ...
                if len(all_cols) >= 2:
                    ...
                    if len(corr_settings['columns']) >= 2:
                        ...
                        st.write(
                            "Spearman: Rank correlation, resistant to outliers")
                        st.write(
                            "Kendall: Another rank correlation, better for small samples")

                        corr_settings['alpha'] = st.slider(
                            "Significance level (α):", 0.01, 0.10, 0.05, key="corr_alpha")
                    else:
                        st.warning(
                            "Please select at least two columns for correlation analysis.")
                else:
                    st.warning(
                        "Need at least two columns for correlation analysis.")
                ...
                st.write(
                    "Calculate overall p-values for each parameter and system-wise p-values.")
                ...
                if len(all_cols) >= 1:
                    ...
                    numeric_cols = [
                        col for col in all_cols if pd.api.types.is_numeric_dtype(df[col])]
                    ...
                    if len(pvalue_settings['columns']) >= 1:
                        ...
                        if pvalue_settings['test_type'] == "One-Sample T-Test":
                            ...
                            categorical_cols = [col for col in all_cols if df[col].dtype == 'object' or col.lower() in [
                            ...
                            if categorical_cols:
                                ...
                                st.warning(
                                    "No categorical columns found for grouping. Consider using One-Sample T-Test instead.")
                            ...
                            if categorical_cols:
                                ...
                                st.warning(
                                    "No categorical columns found for grouping. ANOVA requires a grouping variable.")
                        ...
                        if master_cols:
                            st.info(
                                f"System-wise analysis will be performed using: {', '.join(master_cols)}")
                            pvalue_settings['master_columns'] = master_cols
                        else:
                            st.info(
                                "No master columns identified. Only overall p-values will be calculated.")
                            pvalue_settings['master_columns'] = []
                    else:
                        st.warning(
                            "Please select at least one column for p-value analysis.")
                else:
                    st.warning(
                        "No numeric columns available for p-value analysis.")
                ...
                st.write(
                    "We'll automatically analyze your data and suggest appropriate statistical tests.")
                ...
                with col1:
                    ...
                    if 'input_columns' in data_analysis and data_analysis['input_columns']:
                        ...
                        if len(data_analysis['input_columns']) > 10:
                            st.write(
                                f"...and {len(data_analysis['input_columns']) - 10} more")
                    else:
                ...
                with col2:
                    ...
                    if 'process_columns' in data_analysis and data_analysis['process_columns']:
                        ...
                        if len(data_analysis['process_columns']) > 10:
                            st.write(
                                f"...and {len(data_analysis['process_columns']) - 10} more")
                    else:
                ...
                with col3:
                    ...
                    if 'output_columns' in data_analysis and data_analysis['output_columns']:
                        ...
                        if len(data_analysis['output_columns']) > 10:
                            st.write(
                                f"...and {len(data_analysis['output_columns']) - 10} more")
                    else:
                        ...
                        with st.expander(f"{pattern_type.replace('_', ' ').title()} ({len(type_patterns)} found)"):
                            for i, pattern in enumerate(type_patterns[:10]):
                                if pattern_type == 'pre_post_comparison':
                                    st.write(
                                        f"{i+1}. {pattern['pre_column']} → {pattern['post_column']}")
                                elif pattern_type == 'difference_calculation' or pattern_type == 'percentage_calculation':
                                    st.write(
                                        f"{i+1}. {' & '.join([str(col) for col in pattern['input_columns']])} → {pattern['output_column']}")
                                else:
                                    st.write(
                                        f"{i+1}. {pattern['output_column']}")

                            if len(type_patterns) > 10:
                                st.write(
                                    f"...and {len(type_patterns) - 10} more")
                ...
                if master_candidates:
                    st.write(
                        f"Automatically analyzing by master parameters: {', '.join(master_candidates)}")
                    ...
                    for master_param in master_candidates:
                        with st.expander(f"Analysis by {master_param}", expanded=False):
                            ...
                            master_results = analyze_master_parameters(
                                df, [master_param])

                            if not master_results or master_param not in master_results:
                                st.error(
                                    f"Error analyzing master parameter {master_param}.")
                            else:
                                ...
                                if len(groups) == 1:
                                    ...
                                    st.info(
                                        f"⚠️ Note: The dataset only contains one value for {master_param} ({group_name}).")
                                ...
                                if master_param == 'Generated Age Group' or 'age group' in master_param.lower():
                                    ...
                                    sorted_group_items = sorted(
                                        groups.items(), key=lambda x: extract_start_age(x[0]))
                                    print(
                                        f"Age groups sorted order: {[g[0] for g in sorted_group_items]}")
                                else:
                                ...
                                for group_name, group_data in sorted_group_items:
                                    st.write(
                                        f"**{master_param} = {group_name} (n={group_data['count']})**")
                ...
                if 'pair_areas' in st.session_state and st.session_state.pair_areas:
                    for pair_area in st.session_state.pair_areas:
                        pre_cols = [col['name']
                                    for col in pair_area.get('pre', [])]
                        post_cols = [col['name']
                                     for col in pair_area.get('post', [])]
                        ...
                        for i in range(min(len(pre_cols), len(post_cols))):
                            if pre_cols[i] in df.columns and post_cols[i] in df.columns:
                                pre_post_pairs.append(
                                    (pre_cols[i], post_cols[i]))

                if pre_post_pairs:
                    st.write(
                        f"Analyzing {len(pre_post_pairs)} manually configured pre/post pairs...")
                    ...
                    for pre_col, post_col in pre_post_pairs:
                        if pre_col in df.columns and post_col in df.columns:
                            ...
                            if pd.api.types.is_numeric_dtype(df[pre_col]) and pd.api.types.is_numeric_dtype(df[post_col]):
                                ...
                                pct_change = (
                                    diff / df[pre_col]) * 100 if df[pre_col].min() != 0 else None
                    ...
                    if pre_post_results:
                        ...
                        if master_candidates:
                            st.write(
                                "Pre/Post Differences by Master Parameter:")
                            ...
                            for master_param in master_candidates:
                                with st.expander(f"Pre/Post Analysis by {master_param}"):
                                    ...
                                    for group_name, group_df in df.groupby(master_param):
                                        st.write(
                                            f"**{master_param} = {group_name} (n={len(group_df)})**")
                                        ...
                                        for pre_col, post_col in pre_post_pairs:
                                            if pre_col in group_df.columns and post_col in group_df.columns:
                                                ...
                                                if pd.api.types.is_numeric_dtype(group_df[pre_col]) and pd.api.types.is_numeric_dtype(group_df[post_col]):
                                                    # Calculate difference
                                                    diff = group_df[pre_col] - \
                                                        group_df[post_col]
                                                    pct_change = (
                                                        diff / group_df[pre_col]) * 100 if group_df[pre_col].min() != 0 else None
                                        ...
                                        if group_results:
                                            ...
                                            st.write(
                                                "No pre/post results available for this group.")
                else:
                    ...
                    if analyze_all_sheets:
                        try:
                            ...
                            with st.spinner("Loading and analyzing all sheets..."):
                                ...
                                sheets_data, sheets_metadata = load_excel_file(
                                    uploaded_file)

                                # Display sheet information
                                st.subheader("Excel File Structure")
                                st.write(
                                    f"Found {len(sheets_data)} sheets in the workbook.")
                                ...
                                for sheet_name, sheet_df in sheets_data.items():
                                    with st.expander(f"Sheet: {sheet_name} ({sheet_df.shape[0]} rows, {sheet_df.shape[1]} columns)"):
                                        ...
                                        st.write(
                                            f"**Rows:** {sheet_df.shape[0]}, **Columns:** {sheet_df.shape[1]}")

                                        # Analyze data structure for this sheet
                                        sheet_analysis = analyze_data_structure(
                                            sheet_df)
                                        ...
                                        with col1:
                                            ...
                                            if sheet_analysis['numeric_columns']:
                                                ...
                                                for col in sheet_analysis['numeric_columns'][:10]:
                                                    st.write(f"- {col}")
                                                if len(sheet_analysis['numeric_columns']) > 10:
                                                    st.write(
                                                        f"... and {len(sheet_analysis['numeric_columns']) - 10} more")
                                            else:
                                                st.write("None found")
                                        ...
                                        with col2:
                                            ...
                                            if sheet_analysis['categorical_columns']:
                                                # Limit display to first 10
                                                for col in sheet_analysis['categorical_columns'][:10]:
                                                    st.write(f"- {col}")
                                                if len(sheet_analysis['categorical_columns']) > 10:
                                                    st.write(
                                                        f"... and {len(sheet_analysis['categorical_columns']) - 10} more")
                                            else:
                                                st.write("None found")
                                        ...
                                        if 'suggested_analyses' in sheet_analysis:
                                            ...
                                            for analysis in sheet_analysis['suggested_analyses'][:5]:
                                                st.write(
                                                    f"- {analysis.replace('_', ' ').title()}")
                                        else:
                                            st.write(
                                                "No analyses suggested for this sheet.")

                                # Cross-sheet analysis
                                st.subheader("Cross-Sheet Analysis")

                                # Check for common columns across sheets
                                all_columns = {sheet: set(
                                    df.columns) for sheet, df in sheets_data.items()}
                                common_columns = set.intersection(
                                    *all_columns.values()) if all_columns else set()

                                if common_columns:
                                    st.write(
                                        f"**Common Columns Across All Sheets ({len(common_columns)}):**")
                                    # Limit display to first 10
                                    for col in sorted(common_columns)[:10]:
                                        st.write(f"- {col}")
                                    if len(common_columns) > 10:
                                        st.write(
                                            f"... and {len(common_columns) - 10} more")

                                    # Suggest cross-sheet comparison for a selected common column
                                    if len(common_columns) > 0:
                                        selected_column = st.selectbox(
                                            "Select a common column for cross-sheet comparison:", sorted(common_columns))
                                        ...
                                        if numeric_in_all_sheets:
                                            ...
                                            comparison_df = pd.DataFrame(
                                                comparison_data)
                                            st.write(
                                                f"**Comparison of {selected_column} Across Sheets:**")
                                            ...
                                            st.write(
                                                f"Column '{selected_column}' is not numeric in all sheets.")
                                else:
                                    st.write(
                                        "No common columns found across all sheets.")
                                ...
                                st.subheader(
                                    "Train ML Model Using Multi-Sheet Data")
                                with st.expander("Cross-Sheet ML Model Training", expanded=True):
                                    st.write(
                                        "This will analyze relationships between sheets and train a machine learning model")
                                    ...
                                    master_sheet_name, master_info = identify_master_sheet(
                                        sheets_data)

                                    if master_sheet_name:
                                        st.success(
                                            f"Identified master sheet: **{master_sheet_name}** (Score: {master_info['score']})")
                                        ...
                                        if 'info' in master_info:
                                            ...
                                            for key, value in master_info['info'].items():
                                                if key == 'master_in_name' and value:
                                                    ...
                                                    reasons.append(
                                                        "Has compact format (fewer rows, more columns)")
                                                elif key == 'first_sheet' and value:
                                                    reasons.append(
                                                        "Is the first sheet in the workbook")
                                        ...
                                        st.warning(
                                            "Could not automatically identify a master sheet")

                                    # Option to manually select master sheet
                                    allow_manual_selection = st.checkbox(
                                        "Manually select master sheet", value=not bool(master_sheet_name))

                                    if allow_manual_selection:
                                        master_sheet_name = st.selectbox(
                                            "Select the master sheet:",
                                            list(sheets_data.keys()),
                                            index=list(sheets_data.keys()).index(
                                                master_sheet_name) if master_sheet_name else 0
                                        )
                                    ...
                                    if st.button("Train ML Model on All Sheets"):
                                        with st.spinner("Training ML model across sheets..."):
                                            ...
                                            if 'error' in model_results:
                                                st.error(
                                                    f"Error training model: {model_results['error']}")
                                            else:
                                                st.success(
                                                ...
                                                if 'feature_importance' in model_info:
                                                    ...
                                                    importance_data = []
                                                    # Show top 10
                                                    for feature, importance in model_info['feature_importance'][:10]:
                                                ...
                                                if 'merged_data_shape' in model_results:
                                                    st.write(
                                                        f"**Merged Data Shape:** {model_results['merged_data_shape'][0]} rows × {model_results['merged_data_shape'][1]} columns")
                ...
                # Basic data info
                st.write(
                    f"**Rows:** {data_analysis['num_rows']}, **Columns:** {data_analysis['num_cols']}")
                ...
                if data_analysis.get('has_pre_post_data', False):
                    st.write(
                        "**Pre/Post Data Detected!** Paired analysis is possible.")
                ...
                if 'suggested_analyses' in data_analysis:
                    for analysis in data_analysis['suggested_analyses']:
                        if analysis == 'correlation_analysis':
                            ...
                            auto_analysis_options.append(
                                "Correlation Analysis")

                        elif analysis == 'group_comparison':
                            st.write(
                                "- **Group Comparison**: Compare metrics across different groups")
                            auto_analysis_options.append("Group Comparison")

                        elif analysis == 'anova':
                            st.write(
                                "- **ANOVA**: Test differences between group means")
                            ...
                            st.write(
                                "- **Clustering**: Find natural groupings in your data")
                ...
                if auto_analysis_options:
                    ...
                    if manual_config_available:
                        st.success(
                            "Using configuration from your Manual Descriptive Analysis to enhance the ML analysis")
                    ...
                    default_selections = st.session_state.auto_analyses.copy(
                    ) if st.session_state.auto_analyses else []
                    ...
                    if not default_selections or not any(selection in auto_analysis_options for selection in default_selections):
                        default_selections = auto_analysis_options[:min(
                            3, len(auto_analysis_options))]
                    else:
                        # Filter to only include valid options
                        default_selections = [
                            s for s in default_selections if s in auto_analysis_options]

                    # Log before selection
                    st.write(
                        f"**Debug - Default Selections:** {default_selections}")
                    ...
                    # Log after selection
                    st.write(
                        f"**Debug - Updated Auto Analyses:** {st.session_state.auto_analyses}")
                    ...
                    if "Correlation Analysis" in st.session_state.auto_analyses:
                        ...
                        if 'descriptive_analysis_config' in st.session_state and st.session_state.descriptive_analysis_config.get('numeric_columns'):
                            manual_numeric_columns = st.session_state.descriptive_analysis_config.get(
                                'numeric_columns', [])
                            if manual_numeric_columns:
                                st.success(
                                    f"Using numeric columns from manual analysis for correlation calculations")

                                # Prioritize manual numeric columns
                                # Get intersection of manual columns and actual dataframe columns
                                manual_cols_in_df = [
                                    col for col in manual_numeric_columns if col in df.columns]
                        ...
                        if len(num_cols) >= 2:
                            ...
                            if manual_numeric_columns:
                                ...
                                if default_corr_columns:
                                    st.info(
                                        "Using your manually selected numeric columns as defaults for correlation analysis")

                            # If no manual columns, use previous selection if available
                            if not default_corr_columns and st.session_state.corr_columns:
                                default_corr_columns = [
                                    col for col in st.session_state.corr_columns if col in num_cols]

                            # If still no default, use all numeric columns (up to 10 to avoid performance issues)
                            if not default_corr_columns:
                                default_corr_columns = num_cols[:min(
                                    10, len(num_cols))]

                            # Debug info in collapsible area
                            with st.expander("Debug Information", expanded=False):
                                st.write(
                                    f"**Correlation Columns in Session State:** {st.session_state.corr_columns}")
                                st.write(
                                    f"**Default Correlation Columns:** {default_corr_columns}")
                                if manual_numeric_columns:
                                    st.write(
                                        f"**Numeric Columns from Manual Analysis:** {manual_numeric_columns}")
                    ...
                    if "Group Comparison" in st.session_state.auto_analyses:
                        ...
                        if 'descriptive_analysis_config' in st.session_state and st.session_state.descriptive_analysis_config.get('master_columns'):
                            manual_master_columns = st.session_state.descriptive_analysis_config[
                                'master_columns']
                            st.success(
                                f"Using master columns from manual analysis: {', '.join(manual_master_columns)}")

                        # Get potential grouping columns
                        group_cols = data_analysis.get(
                        ...
                        if manual_master_columns:
                            ...
                            st.info(
                                "Group columns include your manually selected master columns plus auto-detected columns")

                        if group_cols:
                            # Debug info in collapsible area
                            with st.expander("Debug Information", expanded=False):
                                st.write(
                                    f"**Group Column in Session State:** {st.session_state.group_col}")
                                if manual_master_columns:
                                    st.write(
                                        f"**Master Columns from Manual Analysis:** {manual_master_columns}")

                            # Use manual master column as default if available
                            default_index = 0
                            if manual_master_columns and manual_master_columns[0] in group_cols:
                                default_index = group_cols.index(
                                    manual_master_columns[0])
                            elif st.session_state.group_col in group_cols:
                                default_index = group_cols.index(
                                    st.session_state.group_col)
                            ...
                            with st.expander("Debug Information", expanded=False):
                                st.write(
                                    f"**Updated Group Column:** {st.session_state.group_col}")
                    ...
                    if "Paired t-test" in st.session_state.auto_analyses:
                        ...
                        if 'descriptive_analysis_config' in st.session_state and st.session_state.descriptive_analysis_config.get('pre_post_pairs'):
                            ...
                            manual_pairs = st.session_state.descriptive_analysis_config[
                                'pre_post_pairs']
                            ...
                            st.success(
                                f"Using {len(pre_post_pairs)} pre/post pairs from your manual analysis configuration")
                        ...
                        # Fall back to the original method if no pairs from descriptive analysis
                            ...
                            st.info(
                                "Using pre/post pairs from current drag & drop configuration")
                            for pair_area in st.session_state.pair_areas:
                                pre_cols = [col['name']
                                            for col in pair_area.get('pre', [])]
                                post_cols = [col['name']
                        ...
                        if pre_post_pairs:
                            pre_cols = list(set([pair[0]
                                            for pair in pre_post_pairs]))
                            post_cols = list(set([pair[1]
                                             for pair in pre_post_pairs]))
                        else:
                            pre_cols = []
                            post_cols = []
                        ...
                        if pre_cols and post_cols:
                            ...
                            with st.expander("Debug Information", expanded=False):
                                st.write(
                                    f"**Pre Column in Session State:** {st.session_state.pre_col}")

                            # Use previously selected pre column if available
                            default_pre_index = 0
                            if st.session_state.pre_col in pre_cols:
                                default_pre_index = pre_cols.index(
                                    st.session_state.pre_col)
                            ...
                            with st.expander("Debug Information", expanded=False):
                                st.write(
                                    f"**Updated Pre Column:** {st.session_state.pre_col}")
                            ...
                            if default_post:
                                ...
                                with st.expander("Debug Information", expanded=False):
                                    st.write(
                                        f"**Post Column in Session State:** {st.session_state.post_col}")
                                    st.write(
                                        f"**Default Post Column:** {default_post}")
                                ...
                                with st.expander("Debug Information", expanded=False):
                                    st.write(
                                        f"**Updated Post Column:** {st.session_state.post_col}")
                    ...
                    if "Regression Analysis" in st.session_state.auto_analyses:
                        ...
                        if len(num_cols) >= 2:
                            ...
                            st.write(
                                f"**Debug - Updated Target Column:** {st.session_state.target_col}")

                            # Filter out the target column for predictors
                            predictor_options = [
                                col for col in num_cols if col != st.session_state.target_col]

                            # Debug predictor variables
                            st.write(
                                f"**Debug - Predictor Columns in Session State:** {st.session_state.predictor_cols}")

                            # Use previously selected predictors if available
                            default_predictors = [
                                p for p in st.session_state.predictor_cols if p in predictor_options]

                            # If no previous selections, default to first 3
                            if not default_predictors:
                                default_predictors = predictor_options[:min(
                                    3, len(predictor_options))]

                            st.write(
                                f"**Debug - Default Predictor Selections:** {default_predictors}")
                            ...
                            # Debug after selection
                            st.write(
                                f"**Debug - Updated Predictor Columns:** {st.session_state.predictor_cols}")
                else:
                    st.warning(
                        "No automatic analyses could be suggested based on your data structure.")
        ...
        if st.session_state.data is not None and st.session_state.selected_columns:
            ...
            if 'pair_areas' in st.session_state and st.session_state.pair_areas:
                ...
                if manual_pairs:
                    ...
                    df, difference_info = calculate_manual_paired_differences(
                        df, manual_pairs)

                    if difference_info['summary']['total_difference_columns_added'] > 0:
                        st.info(
                            f"Generated {difference_info['summary']['total_difference_columns_added']} difference columns from {len(manual_pairs)} manually selected pre/post pairs.")
            ...
            if st.session_state.analysis_type == "Descriptive Statistics":
                ...
                try:
                    ...
                    with st.expander("Debug Information (click to expand)", expanded=False):
                        ...
                        if 'master_columns' in st.session_state:
                            st.write(
                                f"Master columns in session state: {st.session_state.master_columns}")
                            ...
                            for i, col in enumerate(st.session_state.master_columns):
                                ...
                                if col['name'] in df.columns:
                                    st.write(
                                        f"✓ Column '{col['name']}' found in dataframe")
                                else:
                                    st.write(
                                        f"❌ Column '{col['name']}' NOT found in dataframe")
                        else:
                            st.write("No master columns in session state")

                        if 'age_column' in st.session_state and st.session_state.age_column:
                            st.write(
                                f"Age column: {st.session_state.age_column}")
                            if 'generated_age_group_col' in st.session_state:
                                st.write(
                                    f"Generated age group column: {st.session_state.generated_age_group_col}")
                            else:
                                st.write(
                                    "No generated age group column in session state")
                        else:
                    ...
                    if 'master_columns' in st.session_state and st.session_state.master_columns:
                        ...
                        for col in st.session_state.master_columns:
                            ...
                            col_name = col['name'] if isinstance(
                                col, dict) else col
                            ...
                            if col_name in df.columns:
                                ...
                                st.warning(
                                    f"Master column '{col_name}' was configured but not found in the dataset.")
                        ...
                        if 'age_column' in st.session_state and st.session_state.age_column:
                            if 'generated_age_group_col' in st.session_state and st.session_state.generated_age_group_col in df.columns:
                                ...
                                if age_group_col not in master_columns:
                                    ...
                                    st.info(
                                        f"Added generated Age Group column '{age_group_col}' to master parameters")
                    ...
                    if not master_columns:
                        ...
                        age_group_cols = [
                            col for col in df.columns if "Age Group" in col and col not in master_columns]
                    ...
                    with st.expander("Debug Information (click to expand)", expanded=False):
                        ...
                        st.write(
                            f"Master columns for analysis: {master_columns}")

                    if master_columns:
                        st.write(
                            f"Analysis by master parameters: {', '.join(master_columns)}")
                                        ...
                                        if group_stats is not None:
                                            ...
                                            for col in group_stats.columns:
                                                if 'Mean ± Std Dev' in group_stats.index and isinstance(group_stats.loc['Mean ± Std Dev', col], str):
                                                    group_stats[col] = group_stats[col].astype(
                                                        str)
                    ...
                    if pre_post_pairs:
                        st.write(
                            f"Analyzing {len(pre_post_pairs)} manually assigned pre/post pairs...")
                        ...
                        st.info(
                            "No pre/post pairs have been configured. Please use the Excel Drag & Drop configuration to assign pre/post pairs.")
                    ...
                    if master_columns:
                        st.info(
                            "⚠️ Note: Master Parameter Analysis has already been performed in the Tables section above. Please refer to that section for detailed results.")
                    ...
                    with col1:
                        ...
                        for col in all_selected_cols:
                            try:
                                ...
                                skew_data.append(
                                    {'Column': col, 'Skewness': skew_value})
                            except:
                        ...
                        skew_df['Skewness'] = skew_df['Skewness'].apply(
                    ...
                    with col2:
                        ...
                        for col in all_selected_cols:
                            try:
                                ...
                                kurt_data.append(
                                    {'Column': col, 'Kurtosis': kurt_value})
                            except:
                                # For columns where kurtosis can't be calculated
                                kurt_data.append(
                                    {'Column': col, 'Kurtosis': float('nan')})

                        kurt_df = pd.DataFrame(kurt_data)
                        # Convert all values to strings to prevent Arrow conversion errors
                        kurt_df['Kurtosis'] = kurt_df['Kurtosis'].apply(
                            lambda x: "NA" if pd.isna(x) else f"{x:.4f}")
                ...
                if 'ttest_settings' in st.session_state and st.session_state.ttest_settings:
                    ...
                    try:
                        if settings.get('test_type') == "One-sample t-test":
                            ...
                            # Display results
                            st.write(
                                f"**Testing if the mean of {settings['column']} differs from {settings['mu']}**")
                            col1, col2 = st.columns(2)

                            with col1:
                                st.write(
                                    f"Sample mean: {result['sample_mean']:.4f}")
                                st.write(
                            ...
                            if result['p_value'] < settings['alpha']:
                                st.write(
                                    f"With a p-value of {result['p_value']:.4f}, which is less than the significance level of {settings['alpha']}, we **reject** the null hypothesis.")
                                st.write(
                                    f"There is significant evidence that the true mean of {settings['column']} differs from {settings['mu']}.")
                            else:
                                st.write(
                                    f"With a p-value of {result['p_value']:.4f}, which is greater than the significance level of {settings['alpha']}, we **fail to reject** the null hypothesis.")
                                st.write(
                                    f"There is insufficient evidence that the true mean of {settings['column']} differs from {settings['mu']}.")
                            ...
                            if settings.get('method') == "Compare two columns":
                                ...
                                # Display results
                                st.write(
                                    f"**Comparing means of {settings['col1']} and {settings['col2']}**")
                                col1, col2 = st.columns(2)

                                with col1:
                                    st.write(
                                        f"Mean of {settings['col1']}: {result['mean1']:.4f}")
                                    st.write(
                                        f"Mean of {settings['col2']}: {result['mean2']:.4f}")
                                    st.write(
                                ...
                                with col2:
                                    ...
                                    st.write(
                                        f"Degrees of freedom: {result['df']:.1f}")

                                st.write(
                                    f"95% Confidence Interval for difference: ({result['ci_lower']:.4f}, {result['ci_upper']:.4f})")

                                # Interpretation
                                st.subheader("Interpretation")
                                test_name = "Welch's t-test" if not settings['equal_var'] else "Student's t-test"
                                st.write(
                                    f"Using {test_name} ({'unequal' if not settings['equal_var'] else 'equal'} variances):")

                                if result['p_value'] < settings['alpha']:
                                    st.write(
                                        f"With a p-value of {result['p_value']:.4f}, which is less than the significance level of {settings['alpha']}, we **reject** the null hypothesis.")
                                    st.write(
                                        f"There is significant evidence that the means of {settings['col1']} and {settings['col2']} differ.")
                                else:
                                    st.write(
                                        f"With a p-value of {result['p_value']:.4f}, which is greater than the significance level of {settings['alpha']}, we **fail to reject** the null hypothesis.")
                                    st.write(
                                        f"There is insufficient evidence that the means of {settings['col1']} and {settings['col2']} differ.")
                                ...
                                # Display results
                                st.write(
                                    f"**Comparing {settings['num_col']} between {settings['group1']} and {settings['group2']} groups**")
                                col1, col2 = st.columns(2)

                                with col1:
                                    st.write(
                                        f"Mean for {settings['group1']}: {result['mean1']:.4f}")
                                    st.write(
                                        f"Mean for {settings['group2']}: {result['mean2']:.4f}")
                                    st.write(
                                        f"Mean difference: {result['mean_diff']:.4f}")
                                ...
                                if result['p_value'] < settings['alpha']:
                                    st.write(
                                        f"With a p-value of {result['p_value']:.4f}, which is less than the significance level of {settings['alpha']}, we **reject** the null hypothesis.")
                                    st.write(
                                        f"There is significant evidence that the mean {settings['num_col']} differs between {settings['group1']} and {settings['group2']} groups.")
                                else:
                                    st.write(
                                        f"With a p-value of {result['p_value']:.4f}, which is greater than the significance level of {settings['alpha']}, we **fail to reject** the null hypothesis.")
                                    st.write(
                                        f"There is insufficient evidence that the mean {settings['num_col']} differs between {settings['group1']} and {settings['group2']} groups.")
                    ...
                    try:
                        if 'num_col' in settings and 'cat_col' in settings:
                            ...
                            # Display ANOVA table
                            st.write(
                                f"**One-way ANOVA: Effect of {settings['cat_col']} on {settings['num_col']}**")
                            ...
                            if result['p_value'] < settings['alpha']:
                                st.write(
                                    f"With a p-value of {result['p_value']:.4f}, which is less than the significance level of {settings['alpha']}, we **reject** the null hypothesis.")
                                st.write(
                                    f"There is significant evidence that the mean of {settings['num_col']} differs across at least one of the {settings['cat_col']} groups.")
                            else:
                                st.write(
                                    f"With a p-value of {result['p_value']:.4f}, which is greater than the significance level of {settings['alpha']}, we **fail to reject** the null hypothesis.")
                                st.write(
                                    f"There is insufficient evidence that the mean of {settings['num_col']} differs across the {settings['cat_col']} groups.")
                    ...
                    try:
                        ...
                        st.write(
                            f"**Categorical Variables:** {settings['col1']} × {settings['col2']}")
                        st.write(
                            f"**Null Hypothesis (H₀):** The two categorical variables are independent (no association)")
                        st.write(
                            f"**Alternative Hypothesis (H₁):** The two categorical variables are not independent (there is an association)")
                        ...
                        st.write(f"- p-value: {result['p_value']:.4f}")
                        st.write(
                            f"- Cramer's V (effect size): {result['cramers_v']:.4f} ({result['effect_size']} effect)")
                        ...
                        fig = px.imshow(
                            cont_table.values,
                            labels=dict(
                                x=settings['col2'], y=settings['col1'], color="Frequency"),
                        ...
                        if result['p_value'] < settings['alpha']:
                            st.success(
                                f"**Finding:** We reject the null hypothesis (p < {settings['alpha']}). There is a significant association between {settings['col1']} and {settings['col2']}.")

                            # Describe strength of association
                            if result['effect_size'] == "Negligible":
                                st.info(
                                    f"The association is statistically significant but negligible in strength (Cramer's V = {result['cramers_v']:.4f}).")
                            elif result['effect_size'] == "Weak":
                                st.info(
                                    f"The association is statistically significant with weak strength (Cramer's V = {result['cramers_v']:.4f}).")
                            elif result['effect_size'] == "Moderate":
                                st.info(
                                    f"The association is statistically significant with moderate strength (Cramer's V = {result['cramers_v']:.4f}).")
                            else:  # Strong
                                st.info(
                                    f"The association is statistically significant with strong strength (Cramer's V = {result['cramers_v']:.4f}).")

                        else:
                            st.info(
                                f"**Finding:** We fail to reject the null hypothesis (p > {settings['alpha']}). There is not sufficient evidence of an association between {settings['col1']} and {settings['col2']}.")
                ...
                if 'corr_settings' in st.session_state and st.session_state.corr_settings:
                    ...
                    try:
                        if 'columns' in settings and len(settings['columns']) >= 2:
                            ...
                            st.subheader(
                                f"{settings['method'].capitalize()} Correlation Matrix")
                            ...
                            if not result['significant_corrs'].empty:
                                ...
                                st.write(
                                    "No significant correlations found at the specified significance level.")
                        ...
                        st.error(
                            f"Error performing correlation analysis: {str(e)}")
                    ...
                    if 'columns' in settings and len(settings['columns']) >= 1:
                        ...
                        if 'overall_pvalues' in overall_results:
                            ...
                            if len(overall_results['overall_pvalues']) > 0:
                                ...
                                fig_overall = create_pvalue_chart(
                                    overall_results['overall_pvalues'], "Overall P-Values by Parameter")

                                col1, col2 = st.columns([5, 1])
                                with col1:
                                    st.plotly_chart(
                                        fig_overall, use_container_width=True)
                                with col2:
                                    # Heart button for favoriting
                                    is_favorited = chart_id in st.session_state.get(
                                        'chart_favorites', {})
                                    ...
                                    if st.button(
                                        ...
                                        toggle_chart_favorite(
                                            chart_id, fig_overall, "Overall P-Values by Parameter", "P-Value Chart")
                                        st.rerun()
                                ...
                                for master_col, master_data in overall_results['system_pvalues'].items():
                                    ...
                                    for system, system_data in master_data.items():
                                        ...
                                        if len(system_data) > 0:
                                            ...
                                            fig_system = create_pvalue_chart(
                                                system_data, f"P-Values for {master_col} = {system}")

                                            col1, col2 = st.columns([5, 1])
                                            with col1:
                                                st.plotly_chart(
                                                    fig_system, use_container_width=True)
                                            with col2:
                                                # Heart button for favoriting
                                                is_favorited = chart_id in st.session_state.get(
                                                    'chart_favorites', {})
                                                ...
                                                if st.button(
                                                    ...
                                                    toggle_chart_favorite(
                                                        chart_id, fig_system, f"P-Values for {master_col} = {system}", "P-Value Chart")
                                                    st.rerun()
                ...
                if "Correlation Analysis" in st.session_state.auto_analyses and 'auto_corr_settings' in st.session_state:
                    ...
                    try:
                        if 'columns' in auto_corr_settings and len(auto_corr_settings['columns']) >= 2:
                            ...
                            with st.expander("Debug Information", expanded=False):
                                st.write(
                                    f"**Correlation Settings:** {auto_corr_settings}")
                                    ...
                                    if not pd.api.types.is_numeric_dtype(corr_df[col]):
                                        try:
                                            ...
                                            corr_df[col] = pd.to_numeric(
                                                corr_df[col], errors='coerce')
                                        except Exception as e:
                                            st.warning(
                                                f"Column {col} could not be converted to numeric for correlation analysis: {str(e)}")
                            ...
                            # Display correlation matrix
                            st.subheader(
                                f"{auto_corr_settings['method'].capitalize()} Correlation Matrix")
                            ...
                            if not corr_result['significant_corrs'].empty:
                                ...
                                # Show explanation
                                st.info(
                                    f"The table above shows correlations with p-values less than {auto_corr_settings['alpha']}, indicating statistically significant relationships between variables.")
                            else:
                                st.info(
                                    f"No significant correlations found at the significance level α = {auto_corr_settings['alpha']}.")

                            # Store results for later use
                            st.session_state.auto_correlation_results = corr_result
                        else:
                            st.warning(
                                "Need at least two columns for correlation analysis. Please configure the analysis with more columns.")

                    except Exception as e:
                        st.error(
                            f"Error performing automated correlation analysis: {str(e)}")
        ...
        if st.session_state.data is not None and st.session_state.selected_columns:
            ...
            st.session_state.visualization_type = st.selectbox(
                ...
                index=viz_options.index(
                    st.session_state.visualization_type) if st.session_state.visualization_type in viz_options else 0
            )
            ...
            if st.session_state.visualization_type == "Master Parameter Analysis":
                ...
                if not master_columns:
                    st.warning(
                        "No master columns available. Please go back to the configuration and assign master columns.")
                else:
                    # Let user select which master column to analyze
                    selected_master = st.selectbox(
                        "Select master parameter for analysis:", master_columns)
                    ...
                    if not master_results or selected_master not in master_results:
                        st.error(
                            f"Error analyzing master parameter {selected_master}.")
                    else:
                        ...
                        chart_type = st.radio(
                            ...
                            ["Group Statistics Comparison",
                                "Distribution by Groups", "Pre-Post Analysis by Group"]
                        )

                        # Select a numeric column to visualize across groups
                        num_cols = [col for col in df.columns if pd.api.types.is_numeric_dtype(
                            df[col]) and col != selected_master]
                        if not num_cols:
                            st.warning(
                            ...
                            if chart_type in ["Group Statistics Comparison", "Distribution by Groups"]:
                                selected_numeric = st.selectbox(
                                    "Select numerical column to visualize across groups:", num_cols)
                            elif chart_type == "Pre-Post Analysis by Group":
                                # For pre/post analysis
                                pre_post_pairs = st.session_state.get(
                                    'pre_post_pairs', [])
                                ...
                                with col1:
                                    pre_col = st.selectbox(
                                        "Select Pre column:", num_cols, key="pre_col_selector")
                                with col2:
                                    post_col = st.selectbox(
                                        "Select Post column:", num_cols, key="post_col_selector")
                            ...
                            if chart_type in ["Group Statistics Comparison", "Distribution by Groups"]:
                                ...
                                for group_name, group_data in groups.items():
                                    ...
                                    if group_stats is not None and selected_numeric in group_stats.columns:
                                        ...
                                        try:
                                            mean_val = float(
                                                group_stats.loc['Mean', selected_numeric])
                                            std_val = float(
                                                group_stats.loc['Std Dev', selected_numeric]) if 'Std Dev' in group_stats.index else 0
                                            median_val = float(
                                                group_stats.loc['Median', selected_numeric]) if 'Median' in group_stats.index else mean_val
                                            min_val = float(
                                                group_stats.loc['Min', selected_numeric]) if 'Min' in group_stats.index else 0
                                            max_val = float(
                                                group_stats.loc['Max', selected_numeric]) if 'Max' in group_stats.index else 0
                                ...
                                if 'pre_col' not in locals() or 'post_col' not in locals():
                                    st.warning(
                                        "Please select Pre and Post columns for analysis.")
                                else:
                                        ...
                                        if group_df is not None and pre_col in group_df.columns and post_col in group_df.columns:
                                            ...
                                            try:
                                                ...
                                                t_stat, p_value = stats.ttest_rel(
                                                    group_df[pre_col], group_df[post_col])
                                            except:
                                                p_value = float('nan')
                            ...
                            if not summary_data:
                                st.warning(
                                    f"Could not extract valid statistics for {selected_numeric} across {selected_master} groups.")
                            else:
                                ...
                                if viz_subtype == "Bar Chart":
                                    ...
                                    if chart_type in ["Group Statistics Comparison", "Distribution by Groups"]:
                                        stat_options = [
                                            "Mean", "Mean with StdDev", "Median", "Min/Max"]
                                        stat_display = st.selectbox(
                                            "Choose statistic to display:", stat_options)
                                    elif chart_type == "Pre-Post Analysis by Group":
                                        stat_options = [
                                            "Pre-Post Means", "Percent Reduction", "Pre vs Post with P-values"]
                                        stat_display = st.selectbox(
                                            "Choose pre-post analysis display:", stat_options)
                                    ...
                                    if stat_display == "Mean":
                                        ...
                                        fig = px.bar(
                                            ...
                                            labels={
                                                'Group': selected_master, 'Mean': f'Mean {selected_numeric}'},
                                        ...
                                        for i, row in summary_df.iterrows():
                                            fig.add_annotation(
                                                ...
                                                showarrow=False,
                                                font=dict(
                                                    family="Times New Roman, Times, serif", size=10)
                                            )
                                        ...
                                        fig = px.bar(
                                            ...
                                            labels={
                                                'Group': selected_master, 'Median': f'Median {selected_numeric}'},
                                            text='Count',
                                            color='Group',
                                            color_discrete_sequence=px.colors.qualitative.Plotly,
                                            template="plotly_white"  # Clean white template as base
                                        ...
                                        for i, row in summary_df.iterrows():
                                            fig.add_trace(go.Bar(
                                                ...
                                                name=row['Group'],
                                                text=[
                                                    f"Min: {row['Min']:.2f}<br>Max: {row['Max']:.2f}<br>n={row['Count']}"],
                                                hoverinfo='text'
                                            ))
                                    ...
                                    # Pre-Post analysis display options
                                        ...
                                        fig.add_trace(go.Bar(
                                            ...
                                            name=f"{pre_name}",
                                            text=[
                                                f"{row['Pre Mean']:.2f}<br>n={row['Count']}" for i, row in summary_df.iterrows()],
                                            textposition='auto',
                                        ...
                                        fig.add_trace(go.Bar(
                                            ...
                                            name=f"{post_name}",
                                            text=[
                                                f"{row['Post Mean']:.2f}<br>n={row['Count']}" for i, row in summary_df.iterrows()],
                                            textposition='auto',
                                        ...
                                        fig.add_trace(go.Bar(
                                            x=summary_df['Group'],
                                            # Negate to show reduction as positive
                                            y=[-row['Change %']
                                                for i, row in summary_df.iterrows()],
                                            text=[
                                                f"{abs(row['Change %']):.2f}%" for i, row in summary_df.iterrows()],
                                    ...
                                    fig = create_boxplot(
                                        df, selected_numeric, selected_master)
                                    st.plotly_chart(
                                        fig, use_container_width=True)
                ...
                if all_cols:
                    hist_col = st.selectbox(
                        "Select column for histogram:", all_cols)
                    ...
                    is_favorite = chart_id in st.session_state.get(
                        'chart_favorites', {})
                    ...
                    with col2:
                        ...
                        if st.button(heart_icon, key=f"fav_btn_{chart_id}", help="Add/remove from favorites"):
                            toggle_chart_favorite(
                                chart_id, fig, chart_title, "histogram")
                            st.rerun()
                ...
                if all_cols:
                    box_col = st.selectbox(
                        "Select column for box plot:", all_cols)
                    ...
                    if use_grouping:
                        ...
                        grouping_options = [
                            col for col in all_cols if col != box_col]
                        if grouping_options:
                            group_by = st.selectbox(
                                "Select grouping variable:", grouping_options)
                ...
                if len(all_cols) >= 2:
                    ...
                    with col2:
                        ...
                        if st.button(heart_icon, key=f"fav_btn_{chart_id}", help="Add/remove from favorites"):
                            toggle_chart_favorite(
                                chart_id, fig, chart_title, "scatter_plot")
                            st.rerun()
                    ...
                    st.warning(
                        "At least two columns are required for a scatter plot.")
                ...
                if len(all_cols) >= 2:
                    ...
                    if len(corr_cols) >= 2:
                        ...
                        with col2:
                            ...
                            if st.button(heart_icon, key=f"fav_btn_{chart_id}", help="Add/remove from favorites"):
                                toggle_chart_favorite(
                                    chart_id, fig, chart_title, "correlation_heatmap")
                                st.rerun()
                        ...
                        st.info(
                            "Please select at least two columns for correlation heatmap.")
                else:
                    st.warning(
                        "At least two columns are required for a correlation heatmap.")
                ...
                if all_cols:
                    dist_col = st.selectbox(
                        "Select column for distribution:", all_cols)
                    ...
                    with col2:
                        ...
                        if st.button(heart_icon, key=f"fav_btn_{chart_id}", help="Add/remove from favorites"):
                            toggle_chart_favorite(
                                chart_id, fig, chart_title, "distribution_plot")
                            st.rerun()
                ...
                if chart_type == "Categorical Frequency":
                    ...
                    if cat_cols:
                        cat_col = st.selectbox(
                            "Select categorical column:", cat_cols)
                        ...
                        top_n = st.slider(
                            "Show top N categories:", min_value=5, max_value=50, value=10)
                        ...
                        st.warning(
                            "No categorical columns available for frequency bar chart.")
                    ...
                    if cat_cols and num_cols:
                        cat_col = st.selectbox(
                            "Select categorical column (X-axis):", cat_cols)
                        num_col = st.selectbox(
                            "Select numerical column to summarize:", num_cols)
                        ...
                        if summary_stat == "Mean":
                            summary_df = df.groupby(
                                cat_col)[num_col].mean().reset_index()
                            stat_name = "Mean"
                        elif summary_stat == "Median":
                            summary_df = df.groupby(
                                cat_col)[num_col].median().reset_index()
                            stat_name = "Median"
                        elif summary_stat == "Sum":
                            summary_df = df.groupby(
                                cat_col)[num_col].sum().reset_index()
                            stat_name = "Sum"
                        elif summary_stat == "Min":
                            summary_df = df.groupby(
                                cat_col)[num_col].min().reset_index()
                            stat_name = "Min"
                        elif summary_stat == "Max":
                            summary_df = df.groupby(
                                cat_col)[num_col].max().reset_index()
                            stat_name = "Max"

                        # Limit display to top N categories if there are too many
                        if len(summary_df) > 15:
                            top_n = st.slider(
                                "Show top N categories by value:", min_value=5, max_value=30, value=15)
                            summary_df = summary_df.sort_values(
                                num_col, ascending=False).head(top_n)
                            st.info(
                                f"Showing top {top_n} categories by {stat_name.lower()} value.")
                        ...
                        fig = px.bar(
                            ...
                            labels={cat_col: cat_col,
                                    num_col: f"{stat_name} of {num_col}"},
                            template="plotly_white",  # Clean white template as base
                            color_discrete_sequence=px.colors.qualitative.Plotly  # Excel-like colors
                        )
                        ...
                        if not cat_cols:
                            st.warning(
                                "No categorical columns available for grouping.")
                        if not num_cols:
                            st.warning(
                                "No numerical columns available to summarize.")
                ...
                has_datetime = any(pd.api.types.is_datetime64_dtype(
                    df[col]) for col in st.session_state.selected_columns)

                if has_datetime:
                    # Time series line chart
                    datetime_cols = [
                        col for col in st.session_state.selected_columns if pd.api.types.is_datetime64_dtype(df[col])]
                    time_col = st.selectbox(
                        "Select time/date column (X-axis):", datetime_cols)

                    other_cols = [
                        col for col in st.session_state.selected_columns if col != time_col]
                    if other_cols:
                        y_cols = st.multiselect(
                            "Select column(s) to plot:", other_cols, default=[other_cols[0]])

                        if y_cols:
                            # Create a copy of relevant data, sorted by time
                            plot_df = df[[time_col] +
                                         y_cols].sort_values(time_col).copy()
                    ...
                    if len(all_cols) >= 2:
                        ...
                        y_options = [col for col in all_cols if col != x_col]
                        y_cols = st.multiselect(
                            "Select Y-axis column(s):", y_options, default=[y_options[0]])

                        if y_cols:
                            # Create a copy of relevant data, sorted by X
                            plot_df = df[[x_col] +
                                         y_cols].sort_values(x_col).copy()
                        ...
                        st.warning(
                            "At least two columns are required for a line chart.")

            elif st.session_state.visualization_type == "Frequency Analysis":
                st.subheader("Frequency Analysis with Pie Charts")

                st.info(
                    "This visualization shows frequency distribution with both count and percentage pie charts.")

                # Get categorical columns
                categorical_cols = [col for col in df.columns if not pd.api.types.is_numeric_dtype(
                    df[col]) or df[col].nunique() < 10]

                if not categorical_cols:
                    st.warning(
                        "No suitable categorical columns found in the data for frequency analysis.")
                else:
                    # Column selection
                    ...
                    # Display options
                    chart_options = ["Percentage pie chart", "Count pie chart",
                                     "Both pie charts", "Bar chart", "All chart types"]
                    display_option = st.radio(
                        "Select display option:", chart_options)
                    ...
                    display_df = pd.concat(
                        [value_counts_df, total_row], ignore_index=True)
                    ...
                    if isinstance(figures, list):
                        ...
                        if display_option == "Both pie charts" or display_option == "All chart types":
                            ...
                            with col1:
                                ...
                                with chart_col:
                                    st.plotly_chart(
                                        figures[0], use_container_width=True)
                                with heart_col:
                                    is_favorited = chart_id in st.session_state.get(
                                        'chart_favorites', {})
                                    heart_color = "❤️" if is_favorited else "🤍"
                                    if st.button(
                                        ...
                                        toggle_chart_favorite(
                                            chart_id, figures[0], chart_title, "percentage_pie")
                                        st.rerun()
                            ...
                            with col2:
                                ...
                                with chart_col:
                                    st.plotly_chart(
                                        figures[1], use_container_width=True)
                                with heart_col:
                                    is_favorited = chart_id in st.session_state.get(
                                        'chart_favorites', {})
                                    heart_color = "❤️" if is_favorited else "🤍"
                                    if st.button(
                                        ...
                                        toggle_chart_favorite(
                                            chart_id, figures[1], chart_title, "count_pie")
                                        st.rerun()
                            ...
                            if len(figures) > 2:
                                ...
                                with chart_col:
                                    st.plotly_chart(
                                        figures[2], use_container_width=True)
                                with heart_col:
                                    is_favorited = chart_id in st.session_state.get(
                                        'chart_favorites', {})
                                    heart_color = "❤️" if is_favorited else "🤍"
                                    if st.button(
                                        ...
                                        toggle_chart_favorite(
                                            chart_id, figures[2], chart_title, "bar_chart")
                                        st.rerun()
                        else:
                            ...
                            for i, fig in enumerate(figures):
                                ...
                                with heart_col:
                                    ...
                                    if st.button(
                                        ...
                                        toggle_chart_favorite(
                                            chart_id, fig, chart_title, chart_type_key)
                                        st.rerun()
                    else:
                        ...
                        with heart_col:
                            ...
                            if st.button(
                                ...
                                toggle_chart_favorite(
                                    chart_id, figures, chart_title, chart_type_key)
                                st.rerun()
            ...
            with st.expander("Customize Visualization"):
                st.write(
                    "Customize your visualization with the following options:")
                ...
                color_schemes = ["blues", "reds", "greens", "purples",
                                 "oranges", "viridis", "plasma", "inferno", "magma", "cividis"]
        ...
        if st.session_state.data is not None:
            ...
            tab1, tab2, tab3, tab4, tab5 = st.tabs(
                ["📊 Charts & Export", "📋 Data Exploration", "📈 Visualizations", "📊 Statistics", "🔗 Correlations"])
            ...
            with tab1:
                        ...
                        if st.button("📄 Export All Charts to Microsoft Word", type="primary", use_container_width=True):
                            try:
                                with st.spinner("Generating Word document..."):
                                    ...
                                    b64_word = base64.b64encode(
                                        doc_bytes.getvalue()).decode()
                                    st.markdown(f"""
                                    ...
                                    """, unsafe_allow_html=True)
                                    st.success(
                                        "Word document generated successfully! Click the download button above.")
                                    show_confetti()
                            except Exception as e:
                                st.error(
                                    f"Error creating Word document: {str(e)}")
                    ...
                    if st.session_state.get('show_preview', False):
                        ...
                        with st.expander("View All Favorited Charts", expanded=True):
                            for i, (chart_id, chart_item) in enumerate(st.session_state.chart_favorites.items()):
                                ...
                                with col_chart:
                                    st.plotly_chart(
                                        chart_item['figure'], use_container_width=True)
                                with col_remove:
                ...
                include_summary = st.checkbox(
                    "Include Data Summary", value=True)
                include_stats = st.checkbox(
                    "Include Descriptive Statistics", value=True)
                include_viz = st.checkbox("Include Visualizations", value=True)
                include_corr = st.checkbox(
                    "Include Correlation Analysis", value=True)
                ...
                if st.button("Generate HTML Report"):
                    try:
                        ...
                        b64 = base64.b64encode(
                            html_report.encode("utf-8")).decode()
                        ...
                        with st.expander("Preview Report"):
                            st.components.v1.html(
                                html_report, height=500, scrolling=True)

                        # Show confetti and success message with emoji
                        show_confetti()
                        success_emoji = random.choice(EMOJI_DICT["Success"])
                        st.success(
                            f"{success_emoji} Report generated successfully! {success_emoji} Click the link above to download.")
                ...
                if st.button("Export Data"):
                    try:
                                ...
                                if 'analysis_results' in st.session_state:
                                    ...
                                    if result['type'] == 'descriptive' and 'stats_df' in result:
                                        ...
                                        summary_df = pd.DataFrame(
                                            {'Key': ['Analysis Type'], 'Value': [result['type']]})
                                        summary_df.to_excel(
                        ...
                        st.success(
                            f"Data exported successfully as {export_format}! Click the link above to download.")
                ...
                if 'visualization' in st.session_state and 'figure' in st.session_state.visualization:
                    ...
                    if st.button("Export Visualization"):
                        try:
                            ...
                            st.success(
                                f"Visualization exported as {viz_format}! Click the link above to download.")

                        except Exception as e:
                            st.error(
                                f"Error exporting visualization: {str(e)}")
            ...
            if 'age_column' in st.session_state and st.session_state.age_column:
                ...
                st.info(
                    "To generate age groups, please assign an age column in the Drag & Drop interface.")
            ...
            if 'generated_age_group_col' in st.session_state and st.session_state.generated_age_group_col in df.columns:
                ...
                age_group_col = st.session_state.generated_age_group_col
                st.success(
                    f"Using the age groups generated from your configuration: '{age_group_col}'")
            else:
                # Fall back to checking for existing columns in the dataframe
                existing_age_group_cols = [
                    col for col in df.columns if 'age group' in col.lower() or 'Age group' in col]
                generated_age_group_cols = [
                    col for col in df.columns if 'Generated Age Group' in col]
                ...
                if all_age_group_cols:
                    ...
                    if 'Generated Age Group' in all_age_group_cols:
                        ...
                        st.info(
                            f"Using existing age group column: '{age_group_col}' from the Excel file. To create custom age groups, assign an age column in the configuration.")
            ...
            if age_group_col is not None and 'age_column' in st.session_state and st.session_state.age_column:
                ...
                st.info(
                    f"Age groups have been created based on {age_column} (measured in {age_type}).")

                # Display age group distribution
                age_group_counts = df[age_group_col].value_counts(
                ).sort_index()
                ...
                st.info(
                    "Age groups will be created when you click 'Complete Configuration' in the Drag & Drop interface.")
            ...
            if selected_cols:
                try:
                    ...
                    with col2:
                        ...
                        numerical_selected = [
                            col for col in selected_cols if pd.api.types.is_numeric_dtype(df[col])]
                        if numerical_selected:
                            ...
                            for col in numerical_selected:
                                try:
                                    ...
                                    kurt_data.append(
                                        {'Column': col, 'Kurtosis': "NA"})

                            kurt_df = pd.DataFrame(kurt_data)
                            kurt_df['Kurtosis'] = kurt_df['Kurtosis'].astype(
                                str)
                            st.table(kurt_df)
                        else:
                            st.info(
                                "No numerical columns selected for kurtosis calculation.")

                    # Display frequency tables for categorical columns
                    categorical_selected = [
                        col for col in selected_cols if not pd.api.types.is_numeric_dtype(df[col])]
                    if categorical_selected:
                        ...
                        for col in categorical_selected:
                            ...
                            freq_table['Percentage'] = (
                                freq_table['Count'] / len(df) * 100).round(2)
                            st.table(freq_table)
        ...
        with tab3:
            ...
            is_multi_sheet = 'sheets_data' in st.session_state and isinstance(st.session_state.get(
                'sheets_data'), dict) and len(st.session_state.get('sheets_data', {})) > 1

            if is_multi_sheet:
                viz_type = st.selectbox(
                    "Select visualization type:",
                    ["Histogram", "Box Plot", "Scatter Plot", "Correlation Heatmap",
                        "Distribution Plot", "Multi-Sheet Relationships"]
                )
            else:
                viz_type = st.selectbox(
                    "Select visualization type:",
            ...
            if viz_type == "Histogram":
                ...
                if num_cols:
                    hist_col = st.selectbox(
                        "Select column for histogram:", num_cols)
                    bins = st.slider("Number of bins:",
                                     min_value=5, max_value=100, value=20)
                ...
                if num_cols:
                    box_col = st.selectbox(
                        "Select numerical column for box plot:", num_cols)
                    ...
                    if cat_cols:
                        ...
                        if use_grouping:
                            group_by = st.selectbox(
                                "Select grouping variable:", cat_cols)
                ...
                if len(num_cols) >= 2:
                    x_col = st.selectbox(
                        "Select X-axis column:", num_cols, index=0)
                    y_col = st.selectbox(
                        "Select Y-axis column:", num_cols, index=min(1, len(num_cols)-1))
                    ...
                    if cat_cols:
                        ...
                        if use_color:
                            color_by = st.selectbox(
                                "Select color variable:", cat_cols)
                    ...
                    st.warning(
                        "At least two numerical columns are required for a scatter plot.")
                ...
                if len(num_cols) >= 2:
                    ...
                    st.warning(
                        "At least two numerical columns are required for a correlation heatmap.")
                ...
                if num_cols:
                    dist_col = st.selectbox(
                        "Select column for distribution:", num_cols)
                    ...
                    st.warning(
                        "No numerical columns available for distribution plot.")
                ...
                if 'sheets_data' not in st.session_state or not isinstance(st.session_state.get('sheets_data'), dict):
                    st.warning(
                        "No multi-sheet data available. Please upload an Excel file with multiple sheets and select 'Analyze all sheets' in the Analysis tab.")
                else:
                    ...
                    st.write(
                        "This visualization shows how sheets are related to each other based on common columns.")
                    ...
                    try:
                        ...
                        edges = []
                        edge_weights = []
                        sheet_similarities = relationships.get(
                            "sheet_similarities", {})
                        ...
                        for i, sheet in enumerate(sheet_names):
                            ...
                            node_y.append(y)
                            node_text.append(
                                f"Sheet: {sheet}<br>Rows: {sheets_data[sheet].shape[0]}<br>Columns: {sheets_data[sheet].shape[1]}")
                    ...
                    foreign_keys = relationships.get(
                    ...
                    if foreign_keys:
                        ...
                        st.info(
                            "No potential foreign key relationships detected.")
        ...
        with tab4:
            ...
            if test_type == "t-Test":
                ...
                if len(all_cols) > 0:
                    ...
                    if ttest_type == "One-sample t-test":
                        ...
                        if st.button("Perform One-sample t-test"):
                            try:
                                result = perform_ttest(
                                    df, col, test_type="one-sample", mu=mu, alpha=alpha)
                                ...
                                if result['p_value'] < alpha:
                                    st.write(
                                        f"With a p-value of {result['p_value']:.4f}, which is less than the significance level of {alpha}, we reject the null hypothesis.")
                                    st.write(
                                        f"This suggests that the true mean is significantly different from {mu}.")
                                else:
                                    st.write(
                                        f"With a p-value of {result['p_value']:.4f}, which is greater than the significance level of {alpha}, we fail to reject the null hypothesis.")
                                    st.write(
                                        f"This suggests that there is not enough evidence to conclude that the true mean is different from {mu}.")
                        ...
                        if comparison_type == "Compare two columns":
                            col1 = st.selectbox(
                                "Select first column:", all_cols, index=0)
                            col2 = st.selectbox(
                                "Select second column:", all_cols, index=min(1, len(all_cols)-1))
                            equal_var = st.checkbox(
                                "Assume equal variance", value=False)
                            alpha = st.slider(
                                "Significance level (α):", 0.01, 0.10, 0.05)
                            ...
                            if st.button("Perform Two-sample t-test (columns)"):
                                try:
                                    ...
                                    st.write("### Results")
                                    st.write(
                                        f"Mean of {col1}: {result['mean1']:.4f}")
                                    st.write(
                                        f"Mean of {col2}: {result['mean2']:.4f}")
                                    st.write(
                                        f"Mean difference: {result['mean_diff']:.4f}")
                                    st.write(
                                        f"t-statistic: {result['t_stat']:.4f}")
                                    ...
                                    if result['p_value'] < alpha:
                                        st.write(
                                            f"With a p-value of {result['p_value']:.4f}, which is less than the significance level of {alpha}, we reject the null hypothesis.")
                                        st.write(
                                            f"This suggests that there is a significant difference between the means of {col1} and {col2}.")
                                    else:
                                        st.write(
                                            f"With a p-value of {result['p_value']:.4f}, which is greater than the significance level of {alpha}, we fail to reject the null hypothesis.")
                                        st.write(
                                            f"This suggests that there is not enough evidence to conclude that the means of {col1} and {col2} are significantly different.")

                                    # Confidence interval
                                    st.write(
                                        f"95% Confidence Interval of the difference: ({result['ci_lower']:.4f}, {result['ci_upper']:.4f})")

                                except Exception as e:
                                    st.error(
                                        f"Error performing t-test: {str(e)}")
                            ...
                            if not cat_cols:
                                ...
                                num_col = st.selectbox(
                                    "Select column to test:", all_cols)
                                group_col = st.selectbox(
                                    "Select categorical column for grouping:", cat_cols)

                                # Get unique values in the categorical column
                                unique_vals = df[group_col].unique()
                                if len(unique_vals) < 2:
                                    st.warning(
                                        f"Column '{group_col}' must have at least 2 unique values for comparison.")
                                else:
                                    if len(unique_vals) > 2:
                                        st.info(
                                            f"Column '{group_col}' has {len(unique_vals)} unique values. t-test requires exactly 2 groups.")

                                    group1 = st.selectbox(
                                        "Select first group:", unique_vals, index=0)
                                    group2 = st.selectbox(
                                        "Select second group:", unique_vals, index=min(1, len(unique_vals)-1))
                                    ...
                                    if st.button("Perform Two-sample t-test (groups)"):
                                        try:
                                            ...
                                            st.write("### Results")
                                            st.write(
                                                f"Mean of {num_col} for {group_col}={group1}: {result['mean1']:.4f}")
                                            st.write(
                                                f"Mean of {num_col} for {group_col}={group2}: {result['mean2']:.4f}")
                                            st.write(
                                                f"Mean difference: {result['mean_diff']:.4f}")
                                            st.write(
                                                f"t-statistic: {result['t_stat']:.4f}")
                                            ...
                                            if result['p_value'] < alpha:
                                                st.write(
                                                    f"With a p-value of {result['p_value']:.4f}, which is less than the significance level of {alpha}, we reject the null hypothesis.")
                                                st.write(
                                                    f"This suggests that there is a significant difference in {num_col} between {group_col}={group1} and {group_col}={group2}.")
                                            else:
                                                st.write(
                                                    f"With a p-value of {result['p_value']:.4f}, which is greater than the significance level of {alpha}, we fail to reject the null hypothesis.")
                                                st.write(
                                                    f"This suggests that there is not enough evidence to conclude that {num_col} differs significantly between {group_col}={group1} and {group_col}={group2}.")
                ...
                if not num_cols:
                    ...
                    num_col = st.selectbox(
                        "Select column (dependent variable):", all_cols)
                    cat_col = st.selectbox(
                        "Select categorical column (factor):", cat_cols)

                    # Check the number of groups in the categorical column
                    unique_groups = df[cat_col].nunique()
                    if unique_groups < 2:
                        st.warning(
                            f"Column '{cat_col}' must have at least 2 groups for ANOVA.")
                    else:
                        alpha = st.slider(
                            "Significance level (α):", 0.01, 0.10, 0.05)
                        ...
                        if st.button("Perform ANOVA"):
                            try:
                                result = perform_anova(
                                    df, num_col, cat_col, alpha)

                                # Display results
                                st.write("### Results")
                                st.write(
                                    f"F-statistic: {result['f_stat']:.4f}")
                                st.write(f"p-value: {result['p_value']:.4f}")
                                ...
                                if result['p_value'] < alpha:
                                    st.write(
                                        f"With a p-value of {result['p_value']:.4f}, which is less than the significance level of {alpha}, we reject the null hypothesis.")
                                    st.write(
                                        f"This suggests that at least one group in {cat_col} has a significantly different mean {num_col} from the others.")
                                else:
                                    st.write(
                                        f"With a p-value of {result['p_value']:.4f}, which is greater than the significance level of {alpha}, we fail to reject the null hypothesis.")
                                    st.write(
                                        f"This suggests that there is not enough evidence to conclude that any group in {cat_col} has a significantly different mean {num_col} from the others.")
                ...
                if len(num_cols) < 2:
                    st.warning(
                        "At least two numerical columns are recommended for correlation analysis.")

                # Allow any column for selection, not just numerical
                corr_cols = st.multiselect(
                    "Select columns for correlation analysis:",
                    all_cols,
                    default=num_cols[:min(
                        3, len(num_cols))] if num_cols else all_cols[:min(3, len(all_cols))]
                )
                ...
                if len(corr_cols) < 2:
                    ...
                    if st.button("Perform Correlation Analysis"):
                        try:
                            result = perform_correlation_analysis(
                                df, corr_cols, method=corr_method, alpha=alpha)
                            ...
                            if not result['significant_corrs'].empty:
                                ...
                                st.write(
                                    "No significant correlations found at α = {}.".format(alpha))

                            # Scatter plot matrix
                            st.write("### Scatter Plot Matrix")
                            st.write(
                                "Generating scatter plot matrix for selected variables...")
                            for i in range(len(corr_cols)):
                                for j in range(i+1, len(corr_cols)):
                                    ...
                                    st.write(
                                        f"Correlation ({corr_method}): {result['corr_matrix'].loc[x_col, y_col]:.4f}, p-value: {result['p_matrix'].loc[x_col, y_col]:.4f}")
                        ...
                        if "Descriptive Statistics" in export_options:
                            ...
                            if all_cols:
                                ...
                                for col in stats_df.columns:
                                    if isinstance(stats_df.loc['Mean ± Std Dev', col], str):
                                        # If Mean ± Std Dev is already a string, we need to convert the whole column to strings
                                        stats_df[col] = stats_df[col].astype(
                                            str)
        ...
        if st.session_state.data is not None:
            ...
            with col1:
                st.metric(
                    "Analysis Speed", "⚡ Fast" if "flash" in selected_model else "🔬 Detailed")
            with col2:
                st.metric(
                    "Model Type", "Gemini Flash" if "flash" in selected_model else "Gemini Pro")
            with col3:
                st.metric("Output Quality",
                          "High" if "pro" in selected_model else "Optimized")
            ...
            if not status['ready']:
                st.warning(
                    "Configure your Gemini API key in the sidebar to enable AI-powered analysis summaries.")
                st.info(
                    "Get your free Gemini API key from Google AI Studio: https://aistudio.google.com/app/apikey")
            else:
                st.success(
                    "AI Analysis Ready - Generate narrative summaries of your statistical results")
                ...
                if not available_analyses:
                    st.info(
                        "No analysis results available yet. Complete some statistical analyses in Steps 3-4 to generate AI summaries.")
                else:
                    ...
                    if st.button("Generate AI Summary", type="primary", use_container_width=True):
                        if not selected_analyses:
                            st.warning(
                                "Please select at least one analysis to include in the summary.")
                        else:
                            ...
                            with st.spinner("Generating AI-powered analysis summary..."):
                                # Format data for AI
                                consolidated_text = format_data_for_ai(
                                    selected_analyses)
                    ...
                    if st.session_state.ai_generated_summary:
                        if st.session_state.ai_generated_summary.startswith("Error:"):
                            ...
                            with col2:
                                ...
                                summary_bytes = st.session_state.ai_generated_summary.encode(
                                    'utf-8')
                                st.download_button(
                                    "Download Summary",
                                    data=summary_bytes,
                                ...
                                try:
                                    ...
                                    doc = Document()
                                    doc.add_heading(
                                        'AI-Generated Analysis Summary', 0)
                                    doc.add_paragraph(
            ...
            st.warning(
                "Please upload data in steps 1-2 to access AI analysis features.")
    # In app.py, inside the main() function:
# After the end of:
#   elif st.session_state.current_step == 8:
#       # ... code for AI-Powered Analysis ...
# Add the following new elif block:

    elif st.session_state.current_step == 9:  # <<< THIS IS YOUR NEW STEP
        # Using emoji_header from your app.py
        st.header(emoji_header("9. Drag Drop Visualization", "✨"))

        if 'data' not in st.session_state or st.session_state.data is None:
            st.warning(
                "Please upload data in Step 1 to use the Drag Drop Visualizer.")
            # Optionally, offer to go to Step 1
            if st.button("Go to Upload Data"):
                st.session_state.current_step = 1
                st.rerun()
            return  # Stop further execution for this step if no data

        df = st.session_state.data
        st.markdown("### Dynamic Chart Builder")
        st.write("Use the controls in the sidebar to configure your chart. This section simulates JavaScript drag-and-drop using standard Streamlit widgets.")

        # --- Configuration UI (Simulated Drag-and-Drop in Sidebar) ---
        with st.sidebar:  # Placing controls in the sidebar
            st.title("Chart Controls ✨")
            st.markdown("---")
            st.markdown("#### Configure Visualization")

            chart_type_selected = st.selectbox(
                "Select Chart Type:",
                # Add more types as your backend function supports them
                ["bar", "line", "scatter", "pie", "histogram", "box"],
                key="dnd_chart_type_selection"
            )

            x_axis_selected = st.selectbox(
                "X-Axis:",
                [None] + df.columns.tolist(),  # [None] allows for no selection
                key="dnd_x_axis_selection"
            )

            y_axes_selected = st.multiselect(
                "Y-Axis(es):",
                df.columns.tolist(),
                key="dnd_y_axes_selection"
            )

            color_by_selected = st.selectbox(
                "Color/Group By (optional):",
                [None] + df.columns.tolist(),
                key="dnd_color_by_selection"
            )

            # (Optional) Add more controls for size, facet_row, facet_col if your
            # generate_dynamic_plotly_chart function supports them.
            # size_by_selected = st.selectbox("Size By (for scatter, optional):", [None] + df.columns.tolist(), key="dnd_size_selection")
            # facet_row_selected = st.selectbox("Facet Row (optional):", [None] + df.columns.tolist(), key="dnd_facet_row_selection")
            # facet_col_selected = st.selectbox("Facet Column (optional):", [None] + df.columns.tolist(), key="dnd_facet_col_selection")

        # Create the configuration dictionary from selections
        dnd_simulated_config = {
            'x_axis': x_axis_selected,
            'y_axes': y_axes_selected,
            'color': color_by_selected
            # 'size': size_by_selected,
            # 'facet_row': facet_row_selected,
            # 'facet_col': facet_col_selected
        }

        # --- Chart Generation and Display ---
        main_display_area = st.container()  # Use a container for the main display

        with main_display_area:
            figure_to_display = None  # Initialize

            if dnd_simulated_config.get('x_axis') and dnd_simulated_config.get('y_axes'):
                # st.write(f"Debug: Generating '{chart_type_selected}' chart...") # For debugging
                # st.write(f"Debug: Config - X='{x_axis_selected}', Y='{y_axes_selected}', Color='{color_by_selected}'") # For debugging

                try:
                    figure_to_display = generate_dynamic_plotly_chart(
                        df=df,
                        chart_config=dnd_simulated_config,
                        chart_type=chart_type_selected
                    )
                except Exception as e:
                    st.error(f"Error generating chart: {e}")
                    # figure_to_display = None # Ensure it's None on error
            else:
                st.info(
                    "Please select an X-axis and at least one Y-axis in the sidebar to generate a chart.")
            ...
            if figure_to_display:
                ...
                image_format_options = ["png", "jpeg", "svg", "pdf"]
                selected_download_format = st.selectbox(
                    "Select Image Format:",
                    image_format_options,
                    key="dnd_image_download_format_main"
                )

                try:
                    if hasattr(figure_to_display, 'to_image'):
                        image_bytes_for_download = figure_to_display.to_image(
                            format=selected_download_format)

                        file_name_for_download = f"dynamic_chart.{selected_download_format}"
                        mime_type_for_download = f"image/{selected_download_format}"
                        if selected_download_format == "pdf":
                            mime_type_for_download = "application/pdf"
                        elif selected_download_format == "svg":
                            mime_type_for_download = "image/svg+xml"

                        st.download_button(
                            label=f"Download as {selected_download_format.upper()}",
                            data=image_bytes_for_download,
                            file_name=file_name_for_download,
                            mime=mime_type_for_download,
                            key="dnd_download_chart_button_main"
                        )
                    else:
                        if dnd_simulated_config.get('x_axis') and dnd_simulated_config.get('y_axes'):
                            st.warning(
                                "Chart object is not available for download. This might be due to an error during chart generation.")

                except Exception as e:
                    st.error(f"Could not prepare chart for download: {e}")
                    st.caption(
                        f"Details: This error might occur if Orca or Kaleido is not installed for static image export with Plotly, or if the chart object is invalid.")

            # Placeholder for Dynamic Table Generation and Display
            # st.markdown("---")
            # st.subheader("Dynamic Table (To be implemented)")
            # if x_axis_selected: # Example condition
            #    # Logic to generate and display a table based on selections
            #    # basic_table_df = df[[x_axis_selected] + y_axes_selected] # Example
            #    # st.dataframe(basic_table_df)
            #    pass

# Ensure this is the last part of your main if/elif chain for current_step
    else:
