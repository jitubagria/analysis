...
def display_drag_drop_ui():
    ...
    if 'data' in st.session_state and st.session_state.data is not None:
        ...
        # Use the original columns from the Excel file instead of the current dataframe columns
        # This ensures we only show columns that were actually in the original file
        if 'original_columns' in st.session_state:
            all_columns = st.session_state.original_columns
        else:
            # Fallback: filter out any previously generated age group columns
            all_columns = [col for col in df.columns.tolist() if not (col.startswith('Generated Age Group') and col != 'Generated Age Group')]
        
        # Debug info in collapsible area
        with st.expander("Debug Information", expanded=False):
            st.write(f"**Original columns from Excel:** {all_columns}")
            st.write(f"**Current dataframe columns:** {df.columns.tolist()}")
        
        # Master Area (Optional)
        st.subheader("Master Area (Optional)")
        st.write("These columns will be available for all analyses.")
        
        # Get previously selected master columns if available
        default_master_cols = []
        if 'master_columns' in st.session_state and st.session_state.master_columns:
            default_master_cols = [col['name'] for col in st.session_state.master_columns if col['name'] in all_columns]
        
        # Debug info in collapsible area
        with st.expander("Debug Information", expanded=False):
            st.write(f"**Default Master Columns:** {default_master_cols}")
        
        master_cols = st.multiselect(
            "Select columns for Master Area:",
            all_columns,
            default=default_master_cols
        )
        
        # Age Field (Optional) 
        st.subheader("Age Field (Optional)")
        
        # Get previously selected age column if available
        default_age_col = "None"
        if 'age_column' in st.session_state and st.session_state.age_column:
            if st.session_state.age_column['name'] in all_columns:
                default_age_col = st.session_state.age_column['name']
        
        # Calculate the index directly
        if default_age_col in all_columns:
            age_index = 1 + all_columns.index(default_age_col)  # Add 1 because "None" is at index 0
        else:
            age_index = 0  # Default to "None"
        
        # Debug info in collapsible area
        with st.expander("Debug Information", expanded=False):
            st.write(f"**Default Age Column:** {default_age_col}")
            st.write(f"**Age Index:** {age_index}")
        ...
        if age_col != "None":
            # Get the default age type if available
            default_age_type_idx = 0  # Default to "Years"
            ...
            if 'age_type' in st.session_state:
                if st.session_state.age_type in age_type_map:
                    default_age_type_idx = age_type_map[st.session_state.age_type]
            
            # Debug info in collapsible area
            with st.expander("Debug Information", expanded=False):
                st.write(f"**Default Age Type:** {st.session_state.age_type}")
                st.write(f"**Age Type Index:** {default_age_type_idx}")
            
            age_type = st.radio(
                "Age Type:",
                ["Years", "Months", "Days"],
                index=default_age_type_idx,
                horizontal=True
            )
        ...
        # Get previously selected gender column if available
        default_gender_col = "None"
        if 'gender_column' in st.session_state and st.session_state.gender_column:
            if st.session_state.gender_column['name'] in all_columns:
                default_gender_col = st.session_state.gender_column['name']
        
        # Calculate the index directly
        if default_gender_col in all_columns:
            gender_index = 1 + all_columns.index(default_gender_col)  # Add 1 because "None" is at index 0
        else:
            gender_index = 0  # Default to "None"
        
        # Debug info in collapsible area
        with st.expander("Debug Information", expanded=False):
            st.write(f"**Default Gender Column:** {default_gender_col}")
            st.write(f"**Gender Index:** {gender_index}")
        ...
        if st.button("Complete Configuration"):
            ...
            # Debug info in collapsible area
            with st.expander("Debug Information", expanded=False):
                st.write(f"Master columns set to: {master_cols}")
            ...
            if st.session_state.age_column is not None:
                ...
                # First check for and remove any previously generated age group columns
                # This prevents accumulating multiple age group columns
                generated_cols = [col for col in df.columns if col.startswith('Generated Age Group')]
                if generated_cols:
                    with st.expander("Debug Information", expanded=False):
                        st.write(f"**Removing previously generated age group columns:** {generated_cols}")
                    
                    # Create a new dataframe without the generated columns
