    ...
    try:
        ...
        if len(numeric_columns) >= 2:
            ...
            fig = px.imshow(
                ...
                color_continuous_scale='RdBu_r',  # Red-Blue color scale (Excel-like)
                zmin=-1,
                zmax=1,
...
def create_frequency_analysis(df, column, include_percentage=True, sort_by='count', chart_type='pie'):
    ...
    if chart_type in ['pie', 'both']:
        ...
        hover_text = [f"{row[column]}: {row['Count']} ({row['Percentage']})" for _, row in value_counts.iterrows()]
        ...
        fig_percent.update_traces(
            ...
            marker=dict(line=dict(color='white', width=1)),  # White borders between slices
    ...
    if chart_type in ['pie', 'both']:
        ...
        hover_text = [f"{row[column]}: {row['Count']}" for _, row in value_counts.iterrows()]
    ...
    for i, fig in enumerate(figures):
        fig.update_layout(
            legend=dict(
                ...
                bordercolor='rgba(180, 180, 180, 1)',
                borderwidth=1,
                title=dict(text=column, font=dict(family="Times New Roman, Times, serif")),
...
def create_qq_plot(df, column):
    ...
    if pd.api.types.is_numeric_dtype(df[column]):
        try:
            ...
            min_val = min(qq_df['theoretical_quantiles'].min(), qq_df['sample_quantiles'].min())
            max_val = max(qq_df['theoretical_quantiles'].max(), qq_df['sample_quantiles'].max())
